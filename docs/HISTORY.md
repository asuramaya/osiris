...
> greppable archaeology, never context. The same entries live in the graph as
> dated `ref:history-*` nodes — query `consult_canon(q)` / the `canon` composition.
> Do not append here; do not load this into a session.
