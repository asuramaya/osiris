...
VIEWS, with the right view chosen by the data's shape. This is Palantir's multi-modal object
set, seen from the prosumer side. Every Notion item is a page; a database is a collection of
pages with properties; a view is a lens over that collection.
...
*Board/grouped table*, and a `traverse` ego-network is a *graph*. The view follows intent, not
a raw node count. (The Phase-0 fix made `order/take/aggregate → Table`; Phase 3 generalizes it.)
