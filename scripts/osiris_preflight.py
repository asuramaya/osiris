"""Preflight: a recurring audit meant to run reliably without being forgotten.

An earlier reboot scare taught the underlying lesson: perfect operation can conceal a
misconfiguration. The graph ran flawlessly for nine days on storage that was silently
rigged for data loss, and flawlessness was exactly why nobody noticed. This script
re-runs the survival audit on a timer: units enabled+active, containers on restart
policies with NAMED volumes (never anonymous, since an anonymous volume is what nearly
caused data loss previously), backups fresh AND restorable, default-port squatters (the
shadow-DB trap: a listener on :5432/:6379 catches anything launched without the env
override, causing silent wrong-database writes), unpushed-commit exposure, vault
freshness.

Silent when green. On regression it prints the failures (journal) AND puts a brief on
the operator's desk through the normal mailbox, so it reaches a place someone actually
reads rather than a log nobody checks.
...
counts, since a backup that has never been restored is a hope, not a backup.
"""
...
# Following the same precedent as osiris_fleet_glance.py: this script's deferred `from
# src...` imports failed with ModuleNotFoundError on the exact bare invocation its own
...
# on the harness's per-call output-capture file, while MCP/Read kept working (silent to
# anything that doesn't shell out). Root cause: tests/conftest.py's own PID-keyed
# `basetemp = /tmp/pt-<pid>` (a fix for pytest's AF_UNIX socket-path-length bug) opts
# pytest OUT of its own default retention cleanup. pytest only ever prunes stale runs
# under its own auto-generated `pytest-of-<user>/pytest-<N>/` numbering; a
# caller-supplied --basetemp is never revisited by anyone else's run, so every
# `pt-<pid>` tree from every gate/test invocation, fleet-wide, accumulates forever.
# Measured live during the investigation: 92 leftover `pt-<pid>` dirs spanning ~4.7
# hours of ordinary same-day fleet activity already accounted for 233,279 of 235,301
# files under /tmp (99.1%); at real overnight peak concurrency this is squarely why the
# real incident reached ~1M. This alarm is deliberately NOT also the fix (that's a
# separate, larger change: pruning stale `pt-*` trees safely needs to distinguish a
# live run's own directory from an orphaned one, not just an age cutoff). It only
# guarantees the next climb is seen before every shell on the box dies silently.
TMP_INODE_ALARM_PCT = 80.0
# THE DISK GUARD'S READ-SIDE HALF: osiris_disk_guard.py stops a single WRITE from
# landing on a full disk; this is the standing weekly early-warning that catches the
# climb long before any single write is refused. The emergency that motivated the whole
# lane (92% full, ~3 days runway at 44GB/day) was caught by a human noticing, not by a
# check. 15% free (85% used) gives real runway at that measured burn rate before the
# guard above starts refusing writes.
...
def _tmp_inode_pct(path: str = "/tmp") -> float | None:
    """Percent of `path`'s filesystem inodes in use, via `os.statvfs`; no `df` subprocess
...
def collect() -> dict:
    ...
    # `.dump` (osiris_backup.sh's own -Fc switch) is the CURRENT extension; `.sql` still
    # matches so the retention window's last plain-text dumps (kept ~7 days after the
    # switch) don't cause a false "backup_age_h: None" gap the moment this deploys.
...
async def collect_miner() -> dict | None:
    """The sensing-tick vital signs off miner:ticks. Guards against a fail-open cron
    that could run a full day behind a green heartbeat. Returns None when the
...
async def collect_schema_drift() -> str | None:
    """Belt-and-suspenders on top of the boot-time deploy guard: the boot check only ever
    fires ONCE, at a service's own start. A process that booted clean and then drifted
    later (a migration landed on the DB, or got reverted, while the service kept
    running) would never re-check itself. This weekly pass reuses the SAME comparison
    (deploy_guard.check_schema_drift), not a duplicate of the logic, so the two never
...
def _format_round_trip_failure(failures: list[dict[str, str]] | None) -> str | None:
    """Pure message formatting for the soul store's own round-trip proof. Shared by
    `evaluate()` (so a test can construct `m` directly, same convention as every other
    drill-shaped field here) and `main()`'s own --drill-gated call to
    `collect_soul_round_trip_sample` (which, like `drill_pitr`, runs AFTER
    `evaluate(m)`'s single pass, so `m` never actually carries this key at evaluate-time
    in the real flow; this function is what makes both call sites agree on the exact
    wording without a copy-pasted f-string). A skipped-live session is NEVER a failure:
    see `main()`'s own separate, unconditional "skipped live: N" print. The count must
    be reported, never folded silently into a clean pass, but also never treated as a
    defect worth an operator alarm."""
    if not failures:
        return None
...
async def collect_soul_store_coverage(
    ...
    """The soul store's own coverage guarantee: a session on disk and not in the store
    is a preflight failure, never a quiet gap. Every Claude Code transcript this box can
    see, walked with the SAME ClaudeJsonlAdapter.enumerate() call the backfill cron
    already trusts (so this check and that sweep can never disagree on what counts as a
    session), must have a soul_sessions row. Returns the count missing, or None when
    there's nothing to walk (no transcripts root configured/present here: not a
    failure, just nothing to check in this environment).

    Extended to cover Crush sessions as well: the SAME `CrushSqliteAdapter.enumerate()`
    walk `backfill_crush` already trusts, checked against `harness='crush'`
    soul_sessions rows, folded into one combined missing-count so this function's
    contract (an int, or None when nothing to check) never has to change for every
    caller/test that already reads it that way. `root` only ever scopes the
...
def _restore_cmd(name: str, dump_path: str) -> list[str]:
    """-Fc CUSTOM FORMAT NEEDS pg_restore, NEVER psql. A live incident found that
    piping a .dump file into psql failed: pg_dump's own -Fc switch is a BINARY
    container format, not SQL text; only pg_restore reads it. `.sql` (the legacy
    extension, which collect()'s glob still tolerates during the transition) is
    genuine SQL text and still needs psql. Branch on the extension actually on disk
    rather than assuming one format forever."""
...
def drill(newest_dump: str) -> str | None:
    ...
    NEVER THE LIVE CLUSTER. This was codified after a real live incident: a manual
    pg_basebackup restore into a DIFFERENTLY-NAMED database on the SAME live cluster
    still generated real WAL against production, when a drill's whole point is to
    generate zero WAL against the thing being drilled. `_DRILL_CONTAINER_NAME` is a
    hardcoded module constant, not a caller-supplied parameter, precisely so this can
    never drift toward the live container's own name by accident; the assertion below
    is the machine-checked version of that same guarantee, not just a naming
    convention trusted by eye."""
...
def drill_pitr() -> str | None:
    """A base backup plus WAL archiving is a hope, not a backup, until a real restore
    has proven a point in time. This reuses osiris_pitr_drill.py's own run_drill
    against the newest base backup in the vault and an auto-picked live marker
    (pick_and_ensure_marker, so no manually authored marker is needed for an
    unattended weekly run). Quiet (None) when no base backup exists yet in this
    environment: point-in-time recovery not being activated here is not a failure of
...
async def collect_soul_round_trip_sample() -> RoundTripReport:
    """The soul store's own round-trip proof: a backup that has never been restored is
    a hope, not a backup, the same principle this file's own plain-dump `drill()`
    already holds, extended to the soul store. Reuses
    ...
    (soul_lines/soul_sessions carry no jsonb columns), so a bare pool is correct here,
    unlike the mailbox writes that were previously found broken."""
...
# THE BACKLOG BAND: the weekly delta line
...
async def collect_obligation_backlog_weekly() -> dict[str, Any]:
    """THE BACKLOG BAND: fleet total open obligations, the top five carrying
    ...
    `compositions._fn_obligation_backlog` rather than a fourth hand-rolled
...
# THE ORPHAN LAWS: the same weekly-
...
def _format_orphan_weekly_line(m: dict[str, Any]) -> str:
    ...
    delta_text = ("first run, no prior week to compare" if delta is None else
                  f"{'+' if delta >= 0 else ''}{delta} since last week")
    # THE VISIT CLASS: "Agent:N" alone re-introduces the exact fiction the
    # read-side adoption logic exists to stop. orphan_census's own `by_type`
...
async def collect_orphan_weekly() -> dict[str, Any]:
    """THE ORPHAN BAND: fleet total disconnected-object count, the by-type
    breakdown, and the delta since the last time this ran. Reuses
    `compositions.orphan_census` rather than a second hand-rolled query, the
...
# THE TRACEABILITY BAND, beside the orphan band: the SAME weekly-cursor shape as the
# orphan band just above, reusing `compositions.traceability_census` rather than a
...
async def collect_traceability_weekly() -> dict[str, Any]:
    ...
    breakdown, and the delta since the last time this ran. Reuses
    `compositions.traceability_census` rather than a second hand-rolled query,
...
# THE WEEKLY ABSTENTION DIGEST, beside the orphan band: the SAME weekly-cursor shape
# the backlog and orphan bands above already established, a third lever pulled the
# same way rather than a fourth hand-rolled watermark. Reads
# `adoption_meter._hatch_counts`, the `unlinked_because`/`unlinked_because_kind` hatch
# every declare-or-refuse door writes (record_decision, open_thread,
# ingest_reference/ingest_log/ingest_reference_doc, record_practice, and now the
# provenance sweep's own Agent/SoftwareProject/Seat lanes). This is a DIFFERENT
...
# THE MCP SERVER'S OWN LIVENESS ACROSS TIME: smoke checks can pass in the gaps between
# OOM kills, with NRestarts sitting at a nonzero value for hours while nothing reads it,
# a presence check mistaken for a health check. A RAW NRestarts read is a point-in-time
# counter systemd only ever resets on its own terms (a daemon-reload or `systemctl
# reset-failed`), never on a schedule this script controls, so this tracks the DELTA
# since the LAST preflight run (a per-run cursor, unlike the weekly bands above): a
# NRestarts-delta-over-time reading rather than a point read. The journal's own
# killed/OOM lines are read the same way, since-last-run, never the unit's whole
# lifetime (a restart from three weeks ago must not alarm forever). Runs on EVERY
# invocation, not --drill-gated: an alarm-driving check, not a weekly digest. The
# weekly line under --drill reuses this SAME per-run read rather than a second
# cumulative-total tracker, since NRestarts is already its own natural delta.
...
async def brief_operator(fails: list[str]) -> None:
    ...
    Uses `src.db.pool.create_pool`, NOT bare `asyncpg.create_pool`: the
    ...
    failed" split this codebase's own send_message already confesses rather than swallows."""
    from src.db.pool import create_pool
    from src.orchestrator.mailbox import send_message
...
# CHECK ITSELF broken, not the database, and that must alarm, never degrade to a quiet None.
# This exact class of bug once silently passed both weekly checks for a while.
...
def main() -> int:
    ...
    if "--drill" in sys.argv:
        ...
        if report is not None:
            # ALWAYS PRINTED, pass or fail: a skipped-live session must be reported
            # as "skipped live: N", never silently folded into a clean pass.
