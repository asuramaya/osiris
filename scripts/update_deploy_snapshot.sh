#!/bin/sh
# THE DEPLOY SNAPSHOT (thread e29b260c, Thoth mail 12947) — pins ~/.local/bin/osiris at a
# separate worktree checked out to the deployed sha, so the operator's CLI never runs a
# gate's candidate tree. Live incident this closes: `osiris resume` died with SoulKeyMissing
# because ~/.local/bin/osiris was a symlink straight into ~/code/osiris/.venv/bin/osiris — an
# EDITABLE install that reads source out of the main checkout, and gate w362c had a candidate
# branch merged into that same checkout for its test run at that exact moment. The long-lived
# services survive a gate untouched (they run in-process from their own last restart); only
# the CLI shim was reading whatever happened to be on disk in ~/code/osiris right then.
#
# Called ONLY by `osiris deploy`, ONLY after a green restart+smoke (the same law every other
# install_*.sh script here follows — deploy is the one sanctioned hand that writes machine
# files, Thoth ruling msg 6949) — never on a raw restart or reboot, and never idempotent-safe
# to run standalone against an arbitrary sha without that context.
#
...
if git -C "$SNAPSHOT_DIR" rev-parse --git-dir >/dev/null 2>&1; then
    ...
    # refuse outright — clear it first. Nothing here is ever the operator's real code; it's
