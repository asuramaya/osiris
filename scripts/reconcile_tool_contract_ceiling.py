#!/usr/bin/env python3
"""Git merge driver for TOOL_CONTRACT_CEILING_CHARS.

THE COLLISION THIS FIXES, measured not hypothetical: two independent branches both raised
the ratchet constant from the same base (135_077) in parallel, to 135_292 (+215) and
135_189 (+112). Each branch's own dry-run against composer reports clean, since the other
branch doesn't exist yet from either one's perspective, so nothing warns either author.
The real conflict exists only once the second branch lands, and "take the larger of the
two" (the intuitive fix) is provably wrong whenever both changes are purely additive: the
true combined total is base plus both deltas, always strictly greater than either branch's
own number alone.
...
into the working tree, or even the index, by the time it invokes a custom driver for THIS
one conflicting path. Measuring at that moment silently read a stale, still-pre-merge
...
available, with no working-tree timing dependency at all, `ours + theirs - base` is
instant, deterministic, and, for the documented and universal historical pattern of this
ratchet (every past raise is a disjoint addition to a DIFFERENT tool's docstring, never two
branches overlapping the same content), mathematically exact: matches the real
135_077+215+112=135_404 measured by hand precisely.
...
driver's job is narrower than proving correctness unconditionally; it is to never be worse
than a human guessing, and to be provably exact for the failure mode that has actually
happened. scripts/measure_tool_contract.py remains the tool for a human (or CI) to get the
true live number directly, any time, including as a check after a merge like this one.
...
ceiling from the tree isn't a ratchet, it's just a thermometer (the same reasoning applies
to test_render_hygiene.py's _ALLOWLIST). Growth must stay a deliberate, hand-authored,
justified act. This driver doesn't touch that: it fires ONLY at the merge collision point,
and even then only replaces the one line neither author could have gotten right alone
(each measured correctly for a tree that didn't yet contain the other's work), never at
ordinary test time, never for a single-lane raise.
...
FIRST REAL COLLISION, TWO LATENT BUGS FOUND BY IT (2026-08-28, the first time this driver
had actually fired against a real merge): (1) `_const_value` used to re-search `[\\d_]+`
over the WHOLE matched line rather than read a captured group, so on a name containing an
underscore (every name this has ever been pointed at) it grabbed the bare `_` inside the
NAME itself and `int("")` raised straight past this file's own `main()`. Git folded that
into an ordinary content conflict, indistinguishable from two humans editing the same
line, and the traceback would have scrolled past unread had it not been caught by close
review. (2) the line regex required nothing but whitespace after the digits, so a value
trailing a "# MEASURED..." changelog comment (this ratchet's own actual convention on
every recent raise) silently read back as "missing", not present: the exact one-sided
version of the same "value is really there, this driver just can't see it" failure. Both
fixed at the root (a captured group; an optional trailing `#.*`). `main()`'s own parsing
calls are ALSO now wrapped in a `try`/`except` that DECLINES with a named reason on any
OTHER future parse failure this driver has not yet met, rather than raising past the merge
machinery again: the belt, not just the one buckle that broke.
"""
...
def _const_re(name: str) -> re.Pattern[str]:
    # A TRAILING `# comment` IS THE COMMON CASE, NOT AN EDGE CASE (from a real merge, not
    # a hypothesis): every raise this ratchet has actually landed carries a
    # "# MEASURED against the merged tree: N tools." trailing note, a shape one past merge
    # needed a human to write by hand because this regex, before this fix, only matched a
    # bare `NAME = NUMBER` line with nothing after it. A real value (196_635  # MEASURED...)
    # silently read back as "missing" against the old regex, not the crash seen on the
    # other side of that merge (that line happened to be bare), but the identical class of
    # gap: a value that IS present, parsed as absent, purely because of what trails it on
    # the line.
    return re.compile(
...
def _const_value(text: str, name: str) -> int | None:
    """THE BUG THIS REPLACES, first live specimen 2026-08-28, found in a real merge rather
    than a hypothesis: the old version re-searched `[\\d_]+` over the WHOLE matched line
    (`m.group(0)`, e.g. "TOOL_CONTRACT_CEILING_CHARS = 197_348") instead of reading a
    captured group. `re.search` finds the FIRST such run left-to-right, which for any
    constant name containing an underscore (every name this driver has ever been pointed
    at) is the bare `_` inside the NAME itself ("TOOL_CONTRACT..."), not the number.
    `"_".replace("_", "")` is `""`, and `int("")` raised past every caller straight out of
    the merge driver. This was not intermittent: it fired on every single invocation this
    constant's own name shape has ever produced; nothing caught it earlier only because
    this driver had never actually collided in a real merge before. Fixed by reading
    `(?P<value>...)` directly off `_const_re`'s own match: the number was always right
    there in the regex that already located it; there was never a reason to re-derive it
    with a second, unanchored search."""
...
# TWO NUMBERS, ONE FILE, ONE DRIVER: a live proof of the char fix, ten minutes after it
# landed, also surfaced this: a run of several merges left the char ceiling correctly
# reconciled at every step, but `assert len(per_tool) == N` sat nine lines below it,
# touched by only ONE branch at a time. No conflict at all, so git took that branch's side
# with nothing for this driver to even see, and main read 143 while the tree actually
# carried 144. A ratchet with two numbers and one merge driver only protected one of them.
# Fixed by (1) hoisting the bare inline assert into a NAMED constant,
# `TOOL_CONTRACT_EXPECTED_COUNT`, the exact same shape as the char ceiling, and
...
# trusts a textual number, even one no conflict marker ever touched). This is exactly the
# property the count number was missing, and it falls out of the existing mechanism for
# free once the count has a name this driver can find.
...
def main(argv: list[str]) -> int:
    ...
    # capture %A's PRE-merge content first, since `git merge-file` below overwrites it in
    ...
    for name in names:
        # DECLINES, NEVER RAISES: the first live specimen of this, not a hypothesis.
        # `_const_value`'s own digit-extraction bug once raised an unhandled ValueError
        # here, which git's merge machinery folded into an ordinary content conflict
        # indistinguishable from two humans editing the same line: the traceback scrolled
        # past and nothing else recorded that the automation had abdicated. That ONE bug
        # is fixed at its root in `_const_value` above; this `try` is the belt for any
        # OTHER parsing failure this driver has not yet met, same principle as
