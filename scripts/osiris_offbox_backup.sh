#!/usr/bin/env bash
# THE OFF-BOX PUSH: this ships now, parameterized by the repository URL alone, so a
# future decision about the backend can be a one-line config change to a timer's own
# ExecStart, never a rebuild. Copies the vault (dumps, base backups, WAL archive) to
# a SECOND box via restic, using content-defined chunking, dedup, and client-side
# encryption before upload, the correct default the moment data leaves this
# physically-controlled box. `restic init` runs exactly once per repository,
# idempotently: `restic snapshots` against a not-yet-initialized repo fails, and only
# THEN do we init. An already-initialized repo is never touched here, and restic
# itself refuses a double-init anyway.
#
