...
of those got its own seat (see src/orchestrator/forks.py for the full explanation). The
plumbing is fixed going forward; this is the cleanup for the sessions that predate the fix.
...
and every object it authored keeps naming it. The Agent object is left exactly as it stands.
What changes is only the REGISTRY: the anchor now resolves to the real seat, the twin's UNREAD
mail is carried across to that seat (fixing the mailbox while destroying the mail would defeat
the point), and no row is left claiming a heartbeat for a mind that is already counted elsewhere.
The record stops over-counting minds; it does not un-say anything anyone said.
...
which is this codebase's named failure mode. Those are reported and left alone: an identity merge
is review-gated, always (constitution 1).

DRY RUN IS THE DEFAULT. A janitor that cannot be rehearsed is a shredder, and this script's
first rehearsal proposed folding a live agent into its own ancestor, which is the only reason
...
async def survey(pool: asyncpg.Pool, root: Path) -> list[dict[str, str]]:
    ...
    proposed folding one seat into its own ancestor, and it folded a pair of generation
    numbers into each other in both directions at once. It would have collapsed the fleet's
    ...
    for r in rows:
        ...
        # review-gated, ALWAYS). A twin that never claimed a NAME is an unclaimed automount
        # row that no mind ever inhabited, and this script's whole warrant is that it may
        # retract what an automated process wrote.
        #
        # A NAME IS TESTIMONY. Some forks claim an entirely new identity on top of their
        # parent seat; others re-claim their own identity after a fork, starting a second
        # generation counter (which is exactly where skipped generation numerals come from).
        # Every one of those is a deliberate act of identity by a living mind, and folding it
        # would be an inference overruling a declaration: the named failure mode this script
        # exists to avoid committing itself. Those cases are reported, not folded.
        # ORDERED, because a seat can carry more than one `handle` assertion (a rename leaves
        # the old one standing) and an unordered LIMIT 1 answers a DIFFERENT NAME on different
        # runs: it could return one name on one run and a different name for the same agent on
        # the next. A guess that changes its mind between rehearsal and apply is not a
        # rehearsal at all.
...
async def fold(pool: asyncpg.Pool, twins: list[dict[str, str]]) -> None:
    ...
    unread mail would DESTROY the mail in the act of fixing the mailbox (a reply addressed to a
    twin seat would otherwise be lost the moment that seat's anchor moves, since the bug is what
    forced the correspondent to write from the twin in the first place). Read mail is left where
    it is: it was delivered, and the record of who read what is history, not plumbing.
