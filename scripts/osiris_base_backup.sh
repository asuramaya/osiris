#!/usr/bin/env bash
# THE WEEKLY BASE BACKUP (vault lane, item 3): WAL archiving into the vault plus a
# weekly base backup. A pg_basebackup is the physical starting point WAL replay needs
# for point-in-time recovery. WAL archiving alone (this lane's own osiris_archive_wal.sh
# + osiris_backup.sh's own pull) can replay FORWARD from a base, but has no base of its
# own; without one, recovery has nothing to start from and every WAL segment ever
# archived is dead weight.
#
...
# RETENTION: osiris_prune_ladder.py's own DB-dump retention plan (dry-run by default,
# --apply gated the same as everywhere else in this lane): a base backup, unlike a WAL
# segment, IS a complete, independently-restorable unit on its own, exactly the "every
# survivor a full" shape that plan already implements and tests. No second thinning
# implementation here.
