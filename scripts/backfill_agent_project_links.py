#!/usr/bin/env python3
"""Backfill the mint_heir edge leak (measured fleet-wide 2026-08-03).
...
heir but never invalidated the ancestor's, and fold_agent's own move-on-fold step never
covered works_in/governs at all. Both write-side gaps are fixed (mint_heir, folds._move_
agent_estate); this is the one-time repair for the 906 edges that already existed before
those fixes landed. Thin CLI over src.orchestrator.agents.backfill_agent_project_links,
which does the real work via the SAME move_agent_project_links both write-side fixes use.
...
Usage: uv run python scripts/backfill_agent_project_links.py [--apply] [--base agent:<id> ...]
       ...
       lineage bases, following a staged rollout, the same pattern
       backfill_seat_bindings.py already uses)
"""
