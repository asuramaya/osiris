#!/usr/bin/env python3
"""Verification proof, tail end: after `osiris rename-project <old> <new> --apply`, prove
the READ ALIAS survives: both the old and the new project name still resolve, to the SAME
object id (the acceptance bar states that writes filed with repo='<name>' land under the
canonical and read back under both names). Imports
...
rely on to resolve either name to the one canonical object.
...
(wired as its own `step` in run.sh so a broken alias shows up as its own distinct failure,
separate from the rename command itself succeeding or failing)."""
import asyncio
import os
import sys
