"""THE DISK GUARD (the vault lane, item 5): the disk-space emergency that opened this
whole lane (92% full, ~3 days runway at 44GB/day) was caught by a human noticing, not by
any check in this house. This closes that gap on the write side: before osiris_backup.sh
writes a new FULL pg_dump, it asks `has_room` whether the filesystem holding it has at
least the last dump's own size plus a margin free. NO is a refusal to write, never a
prune: pruning is `osiris_prune_ladder.py`'s job, and only ever under `--apply` after a
human clears the printed list. A refusal alarms the desk via osiris_alarm.py so
