"""A mechanical AST sweep for "the callee's own signature already supports batching or
injection, and the caller doesn't use it": the shape behind TWO independent finds made
the same night, by two people who did not know about each other:
...
Excludes compositions.py/winning_props and cli.py/cmd_deploy's own sites, since those two
are already owned elsewhere. This looks for the THIRD through Nth specimen.
...
PATTERN A: BATCHABLE SIGNATURE, CALLED ONCE PER LOOP ITERATION.
  ...
  from it (`f([x])`): a batch-shaped callee invoked once per item when it could take
...
# the two known sites above -- explicitly excluded, not this sweep's to re-report
...
def find_pattern_b(all_sigs: list[FuncSig]) -> list[dict[str, Any]]:
    ...
    override it; that is ordinary DI, not the bug). The actual shape being named: function A
    ALREADY threads OTHER dependencies through its own signature (own injectable_params is
    non-empty, so A has the pattern established), and calls function B directly (B also has
