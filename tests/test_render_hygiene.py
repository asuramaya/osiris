"""A hardcoded HTML page, made unrepresentable by CI.

The composition-rendering mechanism (`row_action`, the write leg) let three pages move
onto it, then closed COMPLETED while four pages (/desk, /mail, /fleet, /overhead) stayed
hand-written HTML in Python. Nobody noticed for months, because nothing failed when the
rule was broken. Restating the rule in prose would recreate exactly the conditions under
which it died; this test is the rule.
...
THE RATCHET: an allowlisted file's violating-function COUNT must match
...
THE FIRST CATCH, before this test even landed: app.py's `roadmap_page`
...
opposite ends within the hour: an unrelated /roadmap-retirement sweep (commit bb86bbe)
...
# on purpose: every entry says why it's here and what retires it.
#
# THESE NUMBERS MOVE DOWNWARD BY HAND, NEVER RECOMPUTED: a ratchet that
# derives its own baseline from the tree is not a ratchet, it is a thermometer. It would
# report the erosion and permit it, which is exactly what happened for months. If you
# are here because this test failed after you ADDED a renderer, the answer is a composition
# op or a Function, not a bump to the number below. If you are here because
...
_ALLOWLIST: dict[str, int] = {
    # chrome.py: /desk /mail /fleet /overhead, an unfinished half of the composition-rendering
    # migration. CURRENT-AND-FALLING, 21 render functions (/canon's own render_composition +
    # its whole _comp_* helper chain retired, a SECOND generic composition renderer in Python,
    # duplicating osiris.js client-side; ratchet moved 26 -> 21, verified by this test failing
    # before the number was lowered).
    ...
    # did. /fleet is the one named exit still open, verified live in /ui: its composition
    # is 721 unfolded Agent rows with no liveness signal.
    ...
    # retire /membrane"), UNROUTED, slated for OUTRIGHT DELETION in a future wave, kept in
    # the tree only because app.py still imports `_e` from it. Expected to reach 0 and
    ...
    for path in sorted(SRC_API.rglob("*.py")):
        ...
        if len(hits) != allowed:
            ...
            violations.append(
                ...
                "added one, that's the violation (express it as a "
