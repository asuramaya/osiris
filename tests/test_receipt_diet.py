"""RECEIPT DIET (a context-bloat priority, dated 2026-09-04): a receipt returns what the
caller needs to ACT, and nothing it did not ask for. prior_art/listener/co_agents/
held_work become opt-in params (send/inbox/mount) rather than always-attached payload,
measured live against a real transcript sample: inbox's own messages carry 131.6
bytes/message of prior_art on average (6.4% of the messages payload), send's
prior_art+listener together are ~38% of its own receipt bytes. orient()'s own
blind_spots list measured as the single largest static field in a 37-call sample of the
same transcript: 154,059 of 751,203 total orient() bytes (~4.2K bytes/call), a
project-level fact, not a per-call work item.

This file is the RATCHET for the read-verb receipt slice (send, inbox, mount, orient)
AND the write-verb slice (record_decision/open_thread/send nags + prior_art): a
byte-per-call CEILING per representative call shape, raised only as a deliberate,
reasoned act (same convention as tests/test_tool_contract_diet.py's own history), never
a reflex.
"""
...
async def test_inbox_seven_message_peek_prior_art_is_opt_in_and_measurably_smaller(
    ...
    """THE SEVEN-MESSAGE PEEK (the reference acceptance shape): a realistic mixed inbox,
    some messages carry real prior_art (a standing decision each message's own body
    echoes), some don't, measured BEFORE (want_prior_art=True, functionally identical to
    every pre-diet inbox() call) and AFTER (the new default) for the exact same seven
    messages. AFTER must be smaller; the ceiling below is the measured AFTER value,
    ratcheted."""
...
async def test_send_dm_receipt_omits_prior_art_and_listener_by_default(
    ...
    await record_decision(
        actions,
        "RULING: the parallax queue drains oldest-first, verbatim.",
...
async def test_orient_omits_blind_spots_list_by_default(actions: Actions) -> None:
    ...
    unless asked. It is the measured largest static (non-work-item) field in a 37-call
    sample of a real transcript (~4.2K bytes/call). The safety fact ('something here is
    unverifiable') survives at near-zero cost; the full list is opt-in."""
    from src import mcp_server as srv
    from src.orchestrator.agents import AgentIdentity
...
# --- WRITE-VERB SLICE (a context-bloat priority): record_decision/open_thread/send:
# echoed summary dropped, protocol_nag/assertion_nag collapsed to a short `nags` code
# (full text moved to describe('nags:<code>')), prior_art slimmed to {id,type,summary}
# (grade/via dropped). resolve_thread/settle needed no change, already lean, same
# finding made for get_status()/settle() in a separate pass.
...
async def test_record_decision_receipt_omits_echoed_summary(actions: Actions) -> None:
    ...
    try:
        out = await srv.record_decision(
            "RULING: the beryl queue drains in arrival order.",
            kind="ruling", ctx=ctx)
    finally:
        srv._pool = saved_pool
        srv._agents.pop(srv._conn_key(ctx), None)
...
# --- CONTEXT DIET ROUND 2: dossier's relationships (measured 76% of its own bytes/call
# on a live 100-row specimen) and roster's 10-paragraph caveats (measured on every
# call regardless of need) are the two confirmed offenders, both dieted the same
# want_*-opt-in shape as orient()'s blind_spots above, since unlike blind_spots these ARE
# the requested content: default collapses to a
...
async def test_dossier_relationships_collapse_to_counts_and_first_ten_by_default(
    ...
    """A hub object with 15 relationships (more than the reference specimen's own 100,
...
# --- THE READ TRIANGLE: get_status(render='text')
...
async def test_get_status_surfaces_a_bare_handoff_pointer_not_the_full_text(
    ...
    """settle.md's own downstream dependency: get_status must know an
