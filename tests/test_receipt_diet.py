...
This file is the RATCHET for the top-ten read-verb bloat (send, inbox, mount — Sekhmet's
own slice) AND the write-verb slice dispatched next (msg 6871, record_decision/
open_thread/send): echoed `summary` dropped from record_decision's own receipt,
protocol_nag/assertion_nag collapsed to a short `nags` code (full text moved to
describe('nags:<code>')), prior_art slimmed to {id,type,summary} (grade/via dropped).
resolve_thread/settle needed no change on inspection — already lean, same finding
Sekhmet's own lane made for get_status(). Byte-per-call CEILINGS per representative call
shape, raised only as a deliberate, reasoned act (same convention as
tests/test_tool_contract_diet.py's own history), never a reflex.
"""
