...
CHARS collisions. No test file existed for this script before a real collision surfaced a
live bug in it, the exact "a check nobody runs is a check that isn't there" shape this
driver's own sibling ratchet exists to prevent, now proven against a script instead of a
docstring.
"""
...
def test_const_value_reads_the_number_not_the_first_underscore_in_the_name() -> None:
    """THE BUG ITSELF: every real constant name this driver has ever
...
def test_main_resolves_the_exact_specimen_shape_from_the_real_collision(
    tmp_path: Path,
) -> None:
    """THE REAL SPECIMEN, reconstructed from history: `ours`' own line carried a trailing
    "# MEASURED..." changelog comment, `theirs`' own line was bare. Against the OLD regex,
    `ours` silently read back as "missing" (a comment-tolerant match never existed) while
    `theirs` (bare, so it DID match) crashed inside `_const_value`'s own digit-search bug,
    exactly the traceback that surfaced, "computing theirs_v". Both defects are fixed; this
    is the shape that must resolve clean, not just avoid crashing."""
...
def test_main_declines_rather_than_raising_when_a_side_fails_to_parse(
    ...
    """THE ACTUAL POINT: a driver that cannot
...
# ═══ two constants, one file, one driver invocation: the actual defect
...
# specimen had the two constants nine lines apart, so a
...
def test_main_reconciles_the_count_even_with_zero_conflict_markers(tmp_path: Path) -> None:
    """THE EXACT SHAPE THAT SLIPPED THROUGH: the char ceiling is untouched
