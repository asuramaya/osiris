...
Thoth mail 12011/12063): the one reader-visible retired-word string in the UI static files
(osiris.js's own bundle-composition summary, "fan out by neighborhood") and the two
...
70c001ec's own NEIGHBORHOOD -> project/focus and district (PROJECT) ruling. The internal
data-model naming (districts/districtByName/buildDistrictModel/landmarks/hub, all bare code
identifiers never shown to a reader, per the census's own comment/identifier "second pass"
split) is deliberately NOT this piece's scope -- mirrors the repo's existing static-source-
guard convention: string/substring proofs against the served JS/HTML, no browser harness.
"""
...
def test_project_pseudo_node_id_template_no_longer_says_district() -> None:
    body = _SPACE_JS.split("function buildDistrictLabelCandidates()", 1)[1][:300]
    assert "__isDistrict: true, id: `project:${d.name}`, name: d.name," in body
...
def test_project_pseudo_node_css_class_no_longer_says_district() -> None:
    ...
    assert 'div.className = nd.__isDistrict ? "lbl project-label"' in body
