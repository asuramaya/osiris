"""THE RECEIPT-VOCABULARY TEST (task #117): the mechanical-rule
half of #117's finding, SHAPE C, VOCABULARY COLLAPSE (a status
...
of the earlier instances hours before still wrote a fourth one, the
strongest evidence that this is a structural defect, not a discipline one, and that
the operator ruling (mechanical enforcement over documentation) is the right response.
...
THE RULE THIS TEST ENFORCES, reusable beyond this one specimen: "do not generalize it
beyond functions that already declare a finite outcome vocabulary" for a status-word
function whose job is collapsing an operation's real
...
def test_the_pre_0044671_fossil_fails_the_disjointness_test() -> None:
    """THE ACCEPTANCE TEST, stated directly: this
    module's own check must fail against the shape that actually shipped and reached
    production as "17/18 pass" before it was caught. Run the SAME scenario table
...
def test_report_verdict_never_reads_identical_for_clean_vs_skipped(capsys: Any) -> None:
    """THE EXACT "17/18 PASS" BUG, the sharpest of the four specimens
    named in that dispatch, stated as a direct comparison rather than trusting the prefix
