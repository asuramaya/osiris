"""The closure miner: a commit may serve as evidence that a transcript's open promise was kept.

An earlier review found nearly a thousand open threads and no clear way to tell which were
actually stale rather than simply undone.

WHAT WAS MEASURED FIRST. Nothing in the pile is stale by AGE: not one open thread is older
than 14 days. The pile exists because Osiris records every promise but witnesses almost no
delivery: the session-miner reads transcripts from roughly 20 projects, while the pulse only
reads commits from seven of them. For 13 of the 20 trees, the graph held no record that
anything was ever done, so nothing could ever close their threads. Running the miner's own
matcher over those trees' real git logs found that 52% of their open threads have a LATER
COMMIT that matches them. The closer was never broken. It was blind.

THE ONE SANCTIONED CROSSING. Constitution #5 keeps miners behind ownership boundaries: a
miner never touches another source's objects, and that is exactly what stopped a commit from
closing a thread a transcript opened. This module is a deliberately approved exception to
that rule, because the git log is ground truth for "was this done" and the graph's own
opinion is not. So the crossing gets its own SOURCE (`closure-miner`), never smuggled in
under the session-miner's name: a boundary that is crossed deliberately must stay legible in
the provenance.

SPLIT BY CONFIDENCE, chosen deliberately over auto-closing everything:
  · STRONG  → resolve, DERIVED, citing the commit, plus a real `resolved_by` edge to that
              commit, not just the property. This closed a second live gap that was still
              widening the topology-derived closure pile after an earlier fix to
              capture.py's own resolve_thread. It is a retrofit, not a new design: the SAME
...
#
# CUT 1 counted shared "distinctive" tokens. The dry run caught it lying in seconds: it wanted
# to close "Order paradox: whether order in text is relational" against a commit about
# init-hooks, on four "distinctive" shared terms, `predicts, rather, than, where`. A hand-kept
# stopword list is a hardcode and it will always leak; adding "rather" to it only moves the leak.
#
# CUT 2 weighted terms by rarity (IDF over the tree's own corpus, so the corpus grades its own
# vocabulary instead of a legislated list). That killed the stopword garbage, but the dry run
# STILL caught it lying: it wanted to close "The daemon's structural assignment and naming"
# against a census-encoder commit on `bearing(5.2), load(4.3)`, and "TRC application + JAX
# backend" against a spectral-diagnostics commit on `track, backend, should`. Precision at the
# strong bar was ~50%. Raising the bar does not save it: "Stage 3 ledger campaigns" scores 21.3
# against a binding-organ commit and is still simply wrong.
#
# THE VERDICT, against the original design here: LEXICAL SIMILARITY IS NOT EVIDENCE THAT WORK
# WAS DONE. Two texts about the same codebase share vocabulary because they are about the same
# codebase. It is enough to ASK a human. It is never enough to assert into an append-only
...
def _evidence(
    ...
    *, weak: float = WEAK_SCORE, strong: float = STRONG_SCORE,
    ...
    Only ONE verdict is "strong", and it is not a similarity score: the commit NAMES the
    thread. Everything else is "weak", a question, never an answer (see the header: two dry
    runs killed the lexical auto-close lane on its own evidence).
    """
    short = thread_id[:8]
...
async def _commits(
    ...
    `since` (the watermark) filters on `o.created_at`, GRAPH INGESTION time, deliberately
    not `authored_date` (the `at` column, read separately below): a commit's author date
    can be backdated or ingested late relative to when it was authored, so ordering the
    watermark by anything but "when THIS graph first saw it" risks silently skipping a
    commit that lands out of author-date order. Ingestion time is monotonic by
    construction (a bigserial-backed `created_at`, never revised)."""
    return [dict(r) for r in await pool.fetch(
...
async def close_by_commits(
    ...
    """Sweep one tree (or every tree): find the commit that witnesses each untouched open
    thread, and grade it. `dry_run=True` (the default, DELIBERATELY) writes NOTHING and
    reports exactly what it would do: a sweep that can close hundreds of threads is a thing
    you read before you let it run, and the first dry run here caught the matcher lying.

    `strong`/`weak` are the rarity bars. They are tuned to a real tree's corpus; a small
    corpus yields small IDF values, so they are parameters rather than constants: the bar
    belongs to the caller who can see the distribution, not to this module.
    ...
    narrowing only the commit axis is sound because any commit older than the watermark was
    already graded against every thread that was ALREADY open-and-untouched at the time it
    was ingested. The one residual gap is a commit whose OWN ingestion lands out of order
    relative to a thread opened in between two runs, which `since` filtering on
    `created_at` (ingestion time, not `authored_date`) already closes; see `_commits`'s own
    docstring. The watermark never advances on a dry run (a read must stay a read) and
    never moves backward (a partial/failed pass leaves it exactly where the last
    SUCCESSFUL one left it, so a retry after a crash re-examines the same commits rather
    ...
    for t in threads:
        ...
        if verdict == "strong":
            ...
            if not dry_run:
                ...
                # own contract (the strong closure witness), so this closure deserves the
                # same traversable edge resolve_thread(artifact=...) mints, not just the
                # property. Idempotent check-then-create, same shape capture.py's own
                # resolved_by minting uses, so a re-run of this sweep over an
                # already-closed thread is a no-op here too.
            ...
            # The weak band (WEAK..STRONG) is topically related and mostly NOT completion: in
            # one repo it matched "User must verify on mobile that stem sampling works" to
            # the commit that BUILT stem sampling, a commit that CREATED that obligation
            # rather than discharging it. Persisting those puts ~90%-wrong guesses at the TOP
            # of the human's triage queue (the review view sorts evidenced threads first),
            # which is worse than saying nothing. They stay in the dry-run REPORT, where a
            # reader can weigh them; the GRAPH only carries what is worth a click.
    ...
    # THE SCAN-BOUNDARY LINE (presence read as coverage): a thread with NO in_repo edge at
    # all is structurally excluded from `_open_untouched_threads`'s own INNER JOIN,
    # permanently, on any cadence. Cheap (one COUNT(*), no join against this sweep's own
    # O(threads) work) and only meaningful fleet-wide: a single-repo scope has no
    # "unreachable" concept, a thread not in THAT repo is simply out of scope, not invisible.
...
async def close_by_commits_scheduled_tick(
    ...
    here, never in the cron wrapper, so a test can exercise the real gate without touching
    arq.

    OFF unless `osiris_closure_miner_enabled`, the kill switch (the same rule
...
# higher, just phrased differently than any keyword list would catch.
#
...
# never guessed into a mint; see `unresolvable` below.
#
# KNOWN, DELIBERATE MISS, not fixed here: this regex extracts the bare hex tail only, so a
# fully-qualified cross-project canonical in prose, such as "thread: 83b35671dfd4 in
# heinrich", loses its "thread:" prefix before `_find_artifact` ever sees it, and the bare
# tail is not a prefix of that Thread's own `id` (a different convention than
# Decision/Commit/Thread/Tension/Practice's own short-id-is-an-id-prefix shape), so it can
# never resolve either way. Confirmed exactly ONE such case in the fleet-wide 77. Fixing
# it would mean a second regex capturing the qualified form and trying `_find_artifact`'s
# canonical-exact branch before the bare-hex branch: real, targeted, and NOT built. One
# confirmed instance does not justify a second extraction path, and the failure mode here
# is silent-safe (stays `unresolvable`, never a wrong mint), not a defect that compounds.
# Revisit only if characterizing a later population finds more than a handful.
_HASH_TOKEN = re.compile(r"\b[0-9a-f]{7,40}\b", re.I)
# A mined-echo cluster's own boilerplate: near-duplicate Thread objects for ONE incident,
# all citing "root cause thread <id>" verbatim. A dedup problem, not a missing-evidence
# problem: excluded here, named as its own smaller, separate task, never entangled with
# this backfill.
...
async def _needs_human_threads(
    ...
    right now. This is closure_health's own `needs_human` bucket (compositions.py),
    independently re-derived here from the same primitives (`thread_closure_status`,
    `_find_artifact`) rather than imported, since this ingest-layer module never reaches
    into the orchestrator's read-composition layer. Recomputed fresh every call, never
    trusted from a prior report: the graph keeps growing, so an artifact that failed to
    resolve yesterday may resolve today."""
...
async def _classify_miss(
    ...
    """Why a token that _find_artifact already refused stays refused. Called only for the
    unresolvable minority, never the 610-wide candidate sweep, so the extra queries here
    stay cheap. `not_a_hash` beats the other two: a token that merely LOOKS like a miss but
    actually names a real Agent is never a commit question at all."""
    for tok in tokens:
...
async def close_by_prose_backfill(
    ...
    `None` (the default) is fleet-wide, on purpose: closure_health's own scope finding
    showed only 40% of the needs_human pile is even osiris's own, and a backfill that only
    served osiris would leave the other 60% untouched in projects with nobody awake to
    run it.

    `dry_run=True` (the default, matching `close_by_commits`' own convention) writes
    NOTHING and reports exactly what it would do.

    THE THREE-WAY REPORT IS THE DELIVERABLE, not the mints:
    ...
    `unresolvable` ITSELF WAS A CONFLATED LABEL, found by hand-characterizing all 77
    fleet-wide unresolvable rows rather than trusting the count: a first pass called every
    miss "the commit-ingestion gap," but that single number silently mixed three unrelated
    facts. `unresolvable_breakdown` (and its matching `*_rows` samples) splits it for real,
    mirroring closure_health's own strong/weak split one layer down:
      - `not_a_hash`: the token names something real in the graph (checked directly,
        ...
        Agent canonical (e.g. a short hex fragment embedded inside an `agent:` canonical
        like "agent:<fragment>-xxxv"). Deliberately never resolved into an edge (an Agent
        is not what closed a thread: `closed_by` already exists for that shape) and
        deliberately never counted toward the ingestion gap below: it was never a missing
      ...
      - `out_of_scope_repo`: the token matches nothing, AND the thread's own project has
        ...
        mining defect: characterizing the 77 found this was the LARGEST bucket by far (one
        project alone was 27 of 77), and it had been silently inflating the headline
        number.
    ...
    for exactly this reason: a `closure-backfill`-sourced resolved_by edge reports
    `strength='weak'` downstream (closure_health's own strong/weak split), never silently
    promoted to the same confidence as a hand-typed artifact=.
    ...
    for t in candidates:
        ...
        if _WAKE_STORM_MARKER in text.lower():
            continue  # its own smaller dedup task, not this one
