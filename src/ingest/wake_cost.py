...
tools, was invisible in the ledger. The miner's cost was known to the cent ($40.49), but the
ghost farm's cost was not: that farm minted 463 agents on projects that had not been opened in
days.

THAT IS THE SAME PATTERN ALREADY FIXED IN THE MINER: a producer whose spend nobody counted, and
which therefore could not be falsified, and which therefore rotted. The wake trigger is dark
today for other reasons (it reads a liveness field that used to lie), but if it ever comes back
it must come back METERED. A hand that cannot be costed is a hand that cannot be governed.
...
This is an OBSERVATION: a parse of files already on disk. No model, no inference, no money, and
it cannot be wrong. It reads each wake ONCE, ever.

WHAT IT DOES NOT DO IS GUESS THE PRICE. The transcript carries TOKENS, not dollars. Tokens are
recorded as a FACT, and cost_usd is left NULL rather than fabricating a number from a price
table that will be stale within the month. An honest gap beats a confident invention.
"""
...
def _receipt(stem: str) -> float | None:
    ...
    New wakes drop it here. Old ones never will, and no amount of cleverness recovers
...
async def meter_wakes(pool: asyncpg.Pool, root: Path, *, limit: int = BATCH) -> dict[str, int]:
    ...
    for path in files:
        ...
        # history on every tick forever, which is free but wasteful.
        await pool.execute(
...
async def meter_receipts(pool: asyncpg.Pool, *, receipts: Path | None = None) -> dict[str, int]:
    ...
    THE FIELD RUN THAT FOUND THIS (2026-07-14, wake 819): the meter's watermark is
    once-per-transcript-file, EVER, correct for a minted wake (a fresh file), and
    structurally blind to a RESUMED one, which appends to an old transcript the historical
...
# ═══ THE OTHER DIMENSION: resource-seconds, beside the vendor's dollars ═══
#
...
# sibling ledger. A hand that cannot be costed is a hand that cannot be governed.
...
async def meter_bodies(pool: asyncpg.Pool, *, receipts: Path | None = None) -> dict[str, int]:
    ...
    OWN mtime, never the sweep's clock: the same discipline as the wake receipts above
