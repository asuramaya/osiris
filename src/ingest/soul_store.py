...
never their read_turns, since that reparses into TurnRow and throws the raw bytes away.
Piece 1 covers the claude-code harness only (line-oriented JSONL); Crush is SQLite-backed
and needs its own verbatim strategy, out of scope here on purpose.
...
RAW BYTES, NOT TEXT (0052): a real transcript can carry a literal NUL byte (0x00),
measured live, thousands of them across two ordinary sessions, which Postgres
`text`/`varchar` columns cannot hold AT ALL, a hard server limitation, not a query bug.
`soul_lines.raw_line` is `bytea`; every line is read, hashed, stored, and rematerialized
as raw bytes end to end, so nothing here can silently mangle a byte the source file
actually carried. `raw_lines()`/`re_materialize()`/`mining_view()` still hand callers
`str` (decoded utf-8, errors='replace' as a last resort): the text-processing consumers
(session mining, JSON parsing) never need to know the storage layer changed.
"""
...
def _warn_soul_key_missing_once(exc: SoulKeyMissing) -> None:
    """THE FIRST KEY MUST COME FROM THE NORMAL CLI: logs ONCE per process, at WARNING,
    never per-call. An ingest loop can call `_encrypt_rows` thousands of times a minute
    on a busy box, and a missing key is one fact, not one log line per row."""
...
class _LazyFernet:
    ...
    KEY MUST COME FROM THE NORMAL CLI). Several read/verify paths below fetch a
    fernet UNCONDITIONALLY up front but only ever use it on content `is_encrypted()`
    says needs it; on a box with NO key configured at all, every row on a fresh
    install is legacy plaintext and `.decrypt()` is never actually reached, so
    constructing this must never raise `SoulKeyMissing` on its own. Only a REAL
    attempt to decrypt genuinely encrypted content with no key available does."""
...
def _split_lines(content: bytes) -> list[bytes]:
    """Every COMPLETE line, `\\n`-terminated, NEVER a trailing partial fragment. This
    guards a live-write safety gap: a session can be resumed and actively appended to
    while an ingest is dispatched against it. A session writes one JSON object per
    line, each ending in `\\n`; if `ingest_path` reads mid-write of the LAST line on
    disk, the bytes on disk end WITHOUT that terminating `\\n`. Returning that
    fragment as a "line" would bake a half-written JSON object permanently into the
    hash chain at its own line_idx: `ingest_path` never revisits an index once
    ingested, so a subsequent re-read (once the write actually finishes) could never
    correct it.

    THE FIX IS STRUCTURAL, NOT AN OCCUPANCY CHECK: rather than asking "is a session
    live right now" (racy by construction, since any occupancy read can go stale
    between the check and the write it gated), only ever trust content up to and
    including the LAST complete `\\n`. Whatever trails it, complete or mid-write, this
    function cannot tell and does not need to; it waits for the next sweep, when the
    same bytes (now `\\n`-terminated, if the write finished) are read fresh and become
    the same line_idx's real content. A quiet file loses nothing (its last real line
    already ends in `\\n`); a file being appended to right now loses only the
    not-yet-complete tail, forever safe to re-read later.
...
def _iter_complete_line_batches(
    ...
    in memory at once (measured ~881MB peak RSS ingesting a 307MB session the old
    whole-file way; a long-lived shared process serving on-demand resume ingests
...
def _park_superseded(target: Path) -> Path | None:
    ...
    This follows an earlier precedent for a similarly shadowing stub, made the
...
def _crush_line_bytes(row: dict[str, Any]) -> bytes:
    """The verbatim unit for a SQLite-backed harness, closing the gap soul store piece
    1 named: "Crush is SQLite-backed... out of scope here on purpose". The equivalent
    of one JSONL line for claude-code, where crush has no line-oriented file at all,
    only `messages` rows in a shared crush.db.
...
def _encrypt_rows(
    ...
    ALREADY-DECRYPTED plaintext and must never see ciphertext), so `line_hash` (index 4)
    stays computed over PLAINTEXT `raw_line` (index 3) by `_hash_rows` itself, unchanged,
    and THIS function only re-wraps the already-hashed tuple's `raw_line` field in
    Fernet ciphertext right before it reaches an INSERT. A key rotation therefore never
    needs to re-derive any hash, only re-encrypt the stored ciphertext column, in place,
    with the chain untouched.

    DEGRADES TO LEGACY PLAINTEXT WHEN NO KEY EXISTS (THE FIRST KEY MUST COME FROM
    THE NORMAL CLI): `get_soul_fernet()`'s own `SoulKeyMissing` used to propagate
    straight out of here, crashing whatever ingest call reached it first; now caught,
    warned ONCE per process (`_warn_soul_key_missing_once`, never once per row/call,
...
def _split_cold_content(content: bytes) -> list[bytes]:
    ...
    happen, since a cold row is only ever folded from a non-empty session) splits to []."""
    if not content:
        return []
...
def _addressable_entries(lines: list[bytes]) -> list[tuple[int, str, str | None]]:
    ...
    `parentUuid` and a genuine link in the harness's own chain (found live, in a
    real splice between two session files, whose false-positive orphan was traced to
    this exact gap) but are never a valid seek target: a caller must never land a
    resume on a hook-injected attachment. Chain-CONTINUITY verification must NOT use
    this function; see `_chain_linked_entries` below, the ONLY correct input to that
    check. Two named functions, not one with a flag: a flag invites exactly the
    conflation that produced the false positive."""
...
def _chain_linked_entries(lines: list[bytes]) -> list[tuple[int, str, str | None]]:
    ...
    `verify_jsonl_chain_boundary`'s first draft used `_addressable_entries` (user/
    assistant only) and reported a false "orphan" on both halves of a real cwd-move
    splice, at the identical relative position (the second real entry after each
    session's own cwd-move cut), traced to `type:"attachment"` lines, which genuinely
    chain (`uuid`/`parentUuid` both real, verified by direct inspection of the raw file)
    but were invisible to that walk. The earlier by-hand proof that first verified this
    splice got the right answer by luck: it checked only the JOIN (B's first entry's
    parentUuid against A's last), never B's own full internal continuity; this function
    is what that protocol should have used throughout. Never use this for a SEEK target
    (`rematerialize_to_disk`'s own `upto`): an attachment is a real chain link but never
    a place a caller should land a resume; `_addressable_entries` stays the only
    accepted seek-target function, unchanged."""
    out: list[tuple[int, str, str | None]] = []
    for idx, raw in enumerate(lines):
        try:
            d = json.loads(raw)
...
def _confession_line(seek_entry: dict[str, Any], withheld: int) -> bytes:
    """The withheld-entries notice, ratified as a standing rule: a guard that states its
    own condition is worth having even on the day it never fires. A synthetic `user`-
    typed entry, `isMeta: true` matching the marker real hook-injected content already
    carries (so osiris's own mining/rendering skips it the same way it already skips
    other isMeta lines), chained onto the seek point as its `parentUuid` so it reads as
    the NEXT thing that happened, not a foreign insert. Never a fabricated compaction:
    this states a fact about what was withheld, never invents what the model itself
    would have summarized.
...
def verify_jsonl_chain_boundary(file_a: str, file_b: str) -> str | None:
    ...
    by a mid-session cwd move (generalized from an earlier by-hand proof on a real
    split pair, so a future pair is never spliced on the strength of a same-session-id
    coincidence alone), or two things that merely share a session id?
    None means clean; otherwise the reason, matching `resume_verdict`'s own shape.

    THREE CHECKS, the same ones that earlier by-hand proof ran, and no more: (1) `file_b`'s
    FIRST chain-linked entry's `parentUuid` must equal `file_a`'s LAST chain-linked
    entry's `uuid`, the literal join point; (2) zero `uuid` overlap between the two
    files, i.e. two genuinely different halves, never the same content ingested twice
    under different names; (3) every entry in `file_b` (after its first, already checked
    by (1)) must have a `parentUuid` resolving to a `uuid` seen earlier in `file_a` or
    `file_b`: "B entries whose parent is in neither file: ZERO". `file_a`'s OWN internal
    structure is trusted, not re-audited: it is a real, harness-written prefix already
    ingested as itself; this function's job is the JOIN, not a second from-scratch chain
    audit of content nobody is disputing.

    WALKS `_chain_linked_entries` (every uuid-bearing line), NEVER `_addressable_entries`
    (user/assistant only, the seek-only function): found live, the first draft used the
    addressable-only walk and reported a false orphan on both halves of a real split pair
    at the identical relative position (the second real entry after each session's own
    cwd-move cut), because `type:"attachment"` lines carry real chain links this walk
    must see. That earlier by-hand protocol got the right answer by luck: it checked
...
class _ChainBroken(Exception):
    ...
    blob it finds. This class exists to consolidate verification into one verified
    reader before encryption, not three copies of the same verify loop. `receipt`
    carries the EXACT `{"error", "verified_through"}` shape every caller already
    returned before this consolidation: each caller catches this and maps it onto its
    own pre-existing contract, so nothing downstream needs to learn a new error shape."""
...
class RoundTripReport:
    """verify_round_trip_sample's own receipt, ratified after a full sweep: `failures`
    is the same shape as before (a list of {"anchor_sid", "error"} dicts); [] still
    means every COMPARED session verified byte-identical. `skipped_live` is a SEPARATE,
    named count of sessions still being actively appended to at sweep time (never
    silently folded into "clean"): a caller must report both, e.g. "0 failures, skipped
    live: 2", not just the failure count. `cwd_rewrites` is a THIRD, separate count: a
    session `heal_slug_transcripts`' own sanctioned re-addressing (mounts.py,
    ...
    `legacy_count`: how many of THIS sample's sessions were still legacy plaintext. The
    round-trip check itself is fallback-tolerant, so a legacy row verifies clean same as
    an encrypted one; this count is the ONLY signal in this receipt that the migration
    (`encrypt_existing_soul_lines`) still has work left, so a caller never reads a clean
    pass as "fully encrypted" when it isn't yet."""
...
class SoulStore:
    ...
    async def _checkpoint(
        ...
        ingest (the first backfill run covers roughly 2,070 files and is expected to be
        interrupted) resumes from its last committed BATCH, not from scratch.

        `conn` (the checkpoint race): pass the SAME connection `ingest_path` just
        inserted this batch's soul_lines rows on, still inside its own open transaction;
        this upsert then commits ATOMICALLY with those rows, never as a second, separate
        write a process death between the two could split. Defaults to `self.pool` (a
        fresh connection, its own implicit transaction) for every other caller that
        doesn't need that guarantee (there is currently only one: none, since
        `splice_sources` already does both writes in one transaction of its own,
    ...
    async def ingest_path(
        ...
        STREAMS THE SOURCE IN BOUNDED BATCHES: measured ~881MB peak RSS ingesting a real
        307MB session the old whole-file way (read the entire file, split every line,
        build one giant rows list, one executemany), not viable in a long-lived shared
        process (osiris-worker, or an on-demand ingest called from inside a resume
        request) serving other work at the same time. This version reads via
        `_iter_complete_line_batches` (bounded chunks, never the whole file in memory)
        and INSERTs + checkpoints progress once per batch of `_INGEST_BATCH_LINES`
        lines, so peak memory is O(batch size) regardless of file size, and an
        interruption mid-file resumes from the last committed batch rather than redoing
        the whole ingest. SAME CONTRACT as before: same return value (count of new
        lines), same idempotence, same signature; every existing resume-path caller
        needs no changes.
        ...
        for batch in _iter_complete_line_batches(source_path):
            ...
            async with self.pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    # between the two used to leave soul_lines rows committed with no
                    # soul_sessions row at all, so _progress/_last_ingested_at both read
                    # the session as never-ingested (confirmed live: 89 orphaned
                    # soul_lines rows, zero soul_sessions rows, for hours). Now both
                    # commit together or neither does.
                    await self._checkpoint(
                        harness, anchor_sid, source_path, idx, prev_hash, conn=conn)
            total_new += len(rows)
        if total_new == 0 and since > 0 and seen == since:
            # THE STORE'S CLOCK IS "LAST SYNCED", NOT "LAST GREW": a full scan that
            # finds the file holds EXACTLY the lines the store already has is a
            # verified sync, and `last_ingested_at` must say so. Before this, a scan
            # with nothing new never touched the row, so a file whose mtime had moved
            # for any reason other than growth (a byte-identical re-materialize, a
            # copy, a `touch`) read as "LIVE, modified more recently than the store's
            # last ingest" forever, and every resume refused to serve the stored
            # record and fell back to a stale copy. Only an EXACT match earns the
            # touch: a shorter file (a stale partial) is never mistaken for the
            # canonical copy, and a longer one already took the checkpoint above.
    ...
    async def forget_and_reingest(
        ...
        """THE SANCTIONED EXCEPTION TO APPEND-ONLY: `heal_slug_transcripts`' own
        re-addressing (mounts.py, `_rewrite_transcript_cwd`) rewrites the `cwd` field on
        every cwd-carrying line of an already-soul-stored transcript, a real, in-place
        edit to lines `ingest_path` has already chained. `ingest_path` can never repair
        this itself: it only ever appends past its own checkpoint
        (`soul_sessions.last_line_idx`), blind to a content change at an EARLIER
        `line_idx`. Pointed at the rewritten file, it would find every touched line
        already "ingested" by index and append nothing, leaving the store's copy
        silently stale forever (the exact divergence `verify_round_trip_sample`'s own
        `cwd_rewrites` class now names but never fixes on its own).
    ...
    async def ingest_crush_session(
        ...
        """VERBATIM INGEST FOR CRUSH: closes the gap soul store piece 1 named on day
        one ("Crush is SQLite-backed... needs its own verbatim strategy, out of scope
        here on purpose"), this IS that strategy. One `messages` row is the verbatim
        unit (`_crush_line_bytes`), hash-chained through the SAME `soul_lines` schema
        and `_hash_rows` every other harness already uses; `harness='crush'` is the
        only thing that differs from `ingest_path`.
    ...
    async def backfill_crush(self, *, limit_per_db: int = 0) -> dict[str, int]:
        ...
        check, since one crush.db holds many sessions, so the FILE'S mtime moving says
    ...
    async def ingest_dsh_session(
        ...
        """VERBATIM INGEST FOR DSH: closes the soul_sessions gap `backfill()`
        deliberately left open. `DshSessionAdapter` is registered in
        `_default_adapters()` for DISCOVERY (transcript_store.py's own harness_turns
        sweep already ingests every DSH session cleanly through it), but `backfill()`
        filters down to `_claude_code_adapters()` only, for the exact reason
        `ingest_crush_session`'s own docstring already names for Crush: `ingest_path`
        raw-byte-splits its SOURCE FILE on `\\n`, which would silently mangle a DSH
        session's zstd-COMPRESSED bytes into garbage "lines" rather than erroring.
        Confirmed live: harness_turns held all six DSH sessions on this box;
        soul_sessions held zero.
    ...
    async def splice_sources(
        ...
        `anchor_sid`: the operation `ingest_path` cannot do. This was caught before
        shipping the wrong claim: `ingest_path`'s own `since` offset slices INTO the
        given file's own line list, which is correct for re-ingesting the SAME file
        after it has grown, and silently WRONG for a second, different file that
        should continue where the first left off (verified live: a 25-line probe
        transcript split at line 12 and fed through two `ingest_path` calls lost 12 of
        the second half's 13 lines, no error, because `since=12` sliced into a 13-line
        file).
        ...
        pair before ingesting anything, refusing the WHOLE splice, writing nothing, on the
        first pair that isn't a genuine continuation. An earlier real split pair passed
    ...
    async def ensure_ingested(
        ...
        """THE MATERIALIZER'S OWN "ENSURE" STEP: discover this session via the same
        adapter `ingest()` uses, ingest whatever is new (idempotent, a no-op past the
        first call), and hand back the `anchor_sid` the caller needs for every store
        query that follows (`resume_diagnostics`, `rematerialize_to_disk`). `ingest()`
        itself only returns a line count, discarding the one thing a caller chaining
        further store calls actually needs. None when no adapter can discover anything
        at this `job_dir`/`cwd` (the session was never anchored on disk at all, a
        genuine absence, not a store miss), distinct from "found and already fully
        ingested" (returns the anchor_sid with 0 new lines, still a real ensure)."""
        for adapter in self._adapters:
            locator = adapter.discover(cwd=cwd, job_dir=job_dir, root=root)
            if locator is not None:
                harness = getattr(locator, "harness", "claude-code")
    ...
    async def resume_diagnostics(
        ...
        here from `soul_lines` instead. The reasoning: check resume_verdict against the
        STORE's own tail measurement, never a disk stat. That is the load-bearing move:
        today's verdict is a measurement of whatever file the hunt happened to land
        ...
        PAGED (same discipline as `verify_chain`/`_stream_verified_write`): a
        ...
        `harness` (the fourth `_HARNESS` occurrence, worse than its five siblings in
        soul_store.py: this one took no override at all): defaults to
        'claude-code' for backward compatibility, same as `verify_chain`'s own note.

        READS THROUGH THE COLD TIER: a session folded into `soul_lines_cold` has no
        `soul_lines` rows left to page through; its whole decompressed content is
        measured in one pass instead (`_accumulate_resume_diagnostics`, the SAME
        accounting the hot loop below uses per page, so a folded session's diagnostics
        are identical to what they were the day before the fold)."""
        # DECRYPTS BEFORE MEASURING: this counts BYTES of the real
    ...
    async def _iter_verified_lines(
        ...
        """THE ONE READ-THROUGH, CHAIN-VERIFYING PRIMITIVE. This consolidates what
        used to be three copies of "hot-paged-SELECT-or-cold-decompress, verify the
        chain" into one, before encryption lands: a wrong or rotated-out key needs ONE
        consistent place to raise, not three. Yields nothing (an empty generator) when
        neither tier holds anything for this session, indistinguishable from a
        zero-iteration loop, matching every caller's own "not ingested" case. A genuine
        break, a hot gap, a hash mismatch, a corrupt cold blob, raises `_ChainBroken`,
        carrying the exact `{"error", "verified_through"}` shape every caller already
        returned before this change; each of the three (now two, after this
        consolidation) wrapper methods below catches it and maps it onto its own
        pre-existing external contract, so nothing downstream learns a new error shape.
        ...
        computed over PLAINTEXT at write time, see `_encrypt_rows`'s own docstring) and
        before yielding.

        ENCRYPTION AT REST: a wrong or rotated-out key raises
        ...
        if cold is not None:
            ...
            if is_encrypted(cold_blob):
                ...
                try:
                    ...
                    # prefix check is a heuristic, not proof); the same honest
    ...
    async def _all_raw_lines(self, harness: str, anchor_sid: str) -> list[bytes] | None:
        ...
        which you hit" is true by construction rather than three separate copies of
        the same if-cold-else-hot branch agreeing by convention. None when neither
        tier holds anything for this session, OR when the chain doesn't verify
        (`_ChainBroken` maps to None here: this function's own contract has never had
        a distinct "corrupt" outcome to report, so a break is folded into the same
    ...
    async def backfill(
        ...
        not hog one tick. The interrupted-backfill case this behavior was built for is
        exactly this: resumable across many ticks by construction, never a special
    ...
    async def verify_chain(self, anchor_sid: str, harness: str = _HARNESS) -> bool:
        ...
        mismatch; True (including vacuously, on zero rows) otherwise. `harness`: a fix
        for a real bug found live, where `ingest_path`/`splice_sources` already accept
        and store a real harness, but this read-back function defaulted to the module
        constant regardless, so a spliced DSH/Crush anchor_sid would report "nothing
        ingested" against a session genuinely fully stored. Defaults to 'claude-code'
        for backward compatibility with every existing caller.

        PAGED: reads `soul_lines` `_REMATERIALIZE_PAGE_LINES`
        rows at a time by `line_idx` range (the same index `soul_lines`' own PK already
        gives this query for free) rather than one `fetch()` of every row, since a
        300MB-class session's raw content should never sit in memory all at once just
        to answer a yes/no.

        THE COLD TIER'S OWN VERSION OF THIS LAW: a folded session has no
        ...
        DECRYPTS BEFORE COMPARING: `line_hash`/`last_hash` were always
    ...
    async def re_materialize(self, anchor_sid: str, harness: str = _HARNESS) -> str | None:
        ...
        module exists to survive (NUL is valid UTF-8). `harness`:
        defaults to 'claude-code' for backward compatibility, see `verify_chain`'s own
        note. READS THROUGH THE COLD TIER via `_all_raw_lines`."""
    ...
    async def raw_lines(self, anchor_sid: str, harness: str = _HARNESS) -> list[str] | None:
        ...
        to `str` at this boundary, same as `re_materialize`. `harness`:
        defaults to 'claude-code' for backward compatibility, see `verify_chain`'s own
        note. READS THROUGH THE COLD TIER via `_all_raw_lines`."""
        lines = await self._all_raw_lines(harness, anchor_sid)
        if lines is None:
    ...
    async def mining_view(
        ...
        applies for its own text extraction, moved here to the projection
        ...
        every tool_use block on that turn, `[]` when none. `harness`:
        defaults to 'claude-code' for backward compatibility, see `verify_chain`'s own
        note. READS THROUGH THE COLD TIER via `_all_raw_lines`."""
        lines = await self._all_raw_lines(harness, anchor_sid)
        if lines is None:
    ...
    async def _verified_lines(
        ...
        promise starts here. `harness`: defaults to 'claude-code'
        ...
        below (a later streaming rewrite of the original whole-file version, prompted
        by the 307MB question): a SEEK (`rematerialize_to_disk`'s own `upto=`) needs to
        know where to STOP before it can decide whether the tail was withheld, genuinely
        a two-pass question the streamed one-pass writer doesn't answer for free, and
        reconciling that properly is real future work, not a same-night rewrite. The
        default path (`upto=None`, every real resume call) never reaches this; it uses
        the streamed writer, at full 307MB-scale memory savings. Only an explicit seek
        (a controlled, human-supervised repair operation, never the hot resume path)
        pays the whole-file cost this function still carries."""
        lines: list[bytes] = []
        try:
            async for line in self._iter_verified_lines(harness, anchor_sid):
                lines.append(line)
    ...
    async def _stream_verified_write(
        ...
        (the old whole-file join measured another ~888MB peak RSS on top of
        ...
        `target`. `harness` (the same parity fix `verify_chain`
    ...
    async def rematerialize_to_disk(
        ...
        soul_lines alone. This is the acceptance test a soul store stands or falls on,
        made durable rather than in-memory-only (re_materialize's own job, unchanged).

        STREAMS the verify-and-write (`_stream_verified_write`, prompted by the 307MB
        ...
        is written at `target`. This is never a silently truncated file that LOOKS
        complete, now guaranteed by writing to a temp file and only renaming it onto
        `target` on a fully clean pass, instead of by simply not calling `_write_dest`
        until the whole content was already assembled. SAME RETURN SHAPE, SAME
        SIGNATURE as before: every existing resume-materializing caller needs no
        changes.
        ...
        recently than what the store knows about (the file's own mtime works as a lease:
        a live process still appending to it keeps moving it forward), so writing over
        it would clobber content the store never saw. `force=True` skips this guard.

        `upto` follows the principle that the store eats whole transcripts, and any
        resume point should be able to be spawned, defaulting to the latest. It SEEKS to
        a specific `user`/`assistant` entry's own `uuid`, the ONLY valid seek targets
        (`_addressable_entries`), deliberately narrower than the full uuid-bearing chain
        `verify_jsonl_chain_boundary` walks (`_chain_linked_entries`): a caller must
        never land a resume on a hook-injected `attachment` line, even though such a
        line is a real chain link. None (default) emits the full latest chain,
        unchanged. Given, emits a genuine byte-exact PREFIX through that entry, and
        REFUSES (an error dict, nothing written) when `upto` matches no addressable
        line, never a silent full-chain fallback.
        ...
        if not force:
            ...
            if (upto is None and existing_mtime is not None
                ...
                # THE RECORD IS ALREADY HERE: osiris should already have a copy of the
                # latest transcript to resume from. The target IS the very file this
                # session was last ingested FROM, and the guard above just proved
                # nothing touched it since. Re-emitting it byte-for-byte is a 300MB
                # no-op at best; at worst a caller read the write's absence as "no
                # record at the spawn cwd" and fell back to a STALE copy in another
                # location. This is a real failure mode seen in practice: the primary
                # location held the full 15.9MB record, a secondary location held a
                # 1.8MB partial, and every resume attempt refused here and spawned at
        ...
        if upto is None:
            ...
            # no seek, and gets the streamed writer at full 307MB-scale memory
    ...
    async def fold_to_cold_tier(
        ...
        """FOLD ONE SESSION INTO THE COLD TIER, following the standing rule that memory
        gets tiers, not deletion: compress every `soul_lines` row for this session into
        one `soul_lines_cold` row (same content, byte-identical to what
        `_stream_verified_write` would have written, a fraction of the storage), then
        delete the per-line rows.
        ...
        # FIRST KEY MUST COME FROM THE NORMAL CLI). It returns the
        ...
        # ENCRYPT-AFTER-GZIP: compression works far better on
    ...
    async def verify_round_trip_sample(
        ...
        """THE ROUND-TRIP PROOF, following the same principle osiris_preflight.py's own
        plain-dump `drill()` holds, extended to the soul store: a backup that's never
        ...
        long-term transcript tarballs) is SKIPPED, never a failure of this
        check; that absence is a separate retention concern (cache with a budget), not
        proof the store's own content is wrong.

        LIVE SESSIONS ARE SKIPPED TOO, COUNTED SEPARATELY FROM A PASS: a full sweep run
        found 2 of 5 raw mismatches were sessions still being actively appended to at
        sweep time (the file's own mtime was newer than last_ingested_at), the SAME
        guard `rematerialize_to_disk` already uses to refuse overwriting a live
        transcript, applied here to avoid comparing a frozen store snapshot against a
        moving target. `RoundTripReport.skipped_live` names the count explicitly: a
        caller must report it as "skipped live: N", never fold it silently into a
        clean pass.

        SAMPLES BOTH TIERS: up to half the sample
        ...
        for row in rows:
            ...
            if mtime is None:
                continue  # gone since ingest, a separate retention concern, not this check's
            if mtime > row["last_ingested_at"]:
                skipped_live += 1  # still being appended to, a moving target, not a defect
                continue
            # LEGACY-PLAINTEXT COUNT: a cheap peek at whichever tier
    ...
    async def verify_crush_round_trip_sample(self, *, n: int = 20) -> RoundTripReport:
        """THE ROUND-TRIP PROOF, CRUSH'S OWN VERSION: `verify_round_
        ...
        already holds: absence is a separate retention concern, never proof the store is
    ...
    async def verify_dsh_round_trip_sample(self, *, n: int = 20) -> RoundTripReport:
        """THE ROUND-TRIP PROOF, DSH'S OWN VERSION: unlike `verify_round_trip_sample`
        above, a plain byte comparison against `source_path` on disk would NEVER
        match: the store holds the DECOMPRESSED lines `ingest_dsh_session` chained,
        while `source_path` names the zstd-COMPRESSED file. Same live-re-read shape
        `verify_crush_round_trip_sample` already uses instead: re-decompress the
        session's own file right now (the SAME `_decompress_bytes` + `_split_lines`
        pair `ingest_dsh_session` itself uses, so this proves the store agrees with
        what a real ingest would produce today, not a second, drifting comparison),
        and compare against what soul_lines (or the cold tier) actually stored.
        ...
        sample` both already hold: absence is a separate retention concern, never proof
        the store is wrong. `skipped_live` stays 0: the harness rewrites the whole file
...
async def encrypt_existing_soul_lines(
    ...
    pass over what's already in the table (measured live: 1,255,671
    ...
    KEYSET-PAGINATED, never `OFFSET` (the same 307MB-question discipline this module
    holds elsewhere, generalized): `(harness, anchor_sid, line_idx)`, `soul_lines`' own
    primary key, orders every page, so a row appended by a concurrent live ingest lands
    AFTER this migration's own moving cursor and is picked up by this same run if it
    arrives in time, or by the next idempotent re-run if it doesn't; an OFFSET-based
    page would silently skip or duplicate rows under exactly that concurrent-write
    condition.
...
async def rewrap_soul_lines_key(
    ...
    """The key rotation's own re-wrap pass (`osiris soul-key rotate`):
    ...
    the old key can ever be safely deleted. This function's own decrypt fallback
    sequence is exactly that distinction: `new_fernet.decrypt` alone (single-key, never
    Multi); success means already on the new primary, skip; failure falls to
    `old_fernet.decrypt`; success means it's a legacy-key row, re-encrypt under
    `new_fernet` and write back; failure under BOTH is a genuine break (plaintext that
    predates encryption entirely, or corruption, or the wrong old key given) and is
    counted, NEVER silently skipped or crashed on. `soul_key_rotate --finish` refuses
    to run while `broken` is nonzero, the same "never a silent gap" law
    ...
    the kind of unforced memory cost this codebase's own standing lesson on sampling
...
def _differs_only_by_cwd(stored: Path, disk: Path) -> bool:
    """THE NAMED-CLASS CHECK: does `disk`
