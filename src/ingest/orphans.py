...
THIS IS A REGRESSION THAT SHIPPED, KNOWINGLY, AND THIS IS THE FIX FOR IT.

Killing the crawl made `sweep_session` the ONLY way the session-miner ever runs, and it
...
picked it up within ten minutes. That was not a decision. It was a consequence, and it needs a fix.

AND IT IS THE ONE CASE THAT PROVES WHY THE SESSION-MINER CANNOT BE THE LIVE AGENT ITSELF.

If the live agent could settle its own session, osiris or a miner would not be needed at all; the
...
def _ended(root: Path, quiet_secs: int, scopes: list[str] | None = None) -> list[Path]:
    ...
    recovered before the freshest. A stat(), nothing more. `scopes` (the session-miner's scope)
    skips scoped-out projects at detection so the per-tick BATCH is never spent
    ...
    for p in paths:
        if p.parent.name.endswith("-osiris-extract"):
            continue  # the session-miner's own scratch sessions: an instrument reading itself
...
async def find_orphans(
    ...
    of which are FACTS. A reaper that acts on a suspicion is the janitor with opinions, and that
    approach was already ruled out.

    `scopes` = the session-miner's project scope: None reads OSIRIS_SENSE_PROJECTS, [] is
    explicitly unscoped.
    """
    if scopes is None:
