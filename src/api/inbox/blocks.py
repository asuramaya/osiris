"""THE FROZEN BLOCK VOCABULARY: slice one of the inbox UI. This closed vocabulary IS the
UI: every surface in this subpackage (and, eventually, agent-composed panes elsewhere)
builds a TREE of these types; render.py is the ONLY code allowed to turn a tree into HTML,
via templates/catalog.html.j2 (one macro per `kind`). A new `kind` here is the one
sanctioned entry point for a new component: reaching for raw HTML anywhere else is the
slop channel this architecture exists to close STRUCTURALLY (a mypy error), not by review
discipline alone.

THREE RULINGS, each load-bearing here:
...
class ActionRow(BaseModel):
    ...
    def _at_most_one_primary(self) -> ActionRow:
        ...
        if primaries > 1:
            raise ValueError(
                f"at most one 'primary' Button per ActionRow, got {primaries}")
        return self


# notify/question/review is a DIFFERENT axis from Badge's status tones: a monochrome
# mono-face glyph, never colored (ruling 3 above).
...
class Region(BaseModel):
    ...
    # shell.html.j2's four named regions: aside is reserved for the fleet-strip module
    # (the next builder in this subpackage), empty in slice one
