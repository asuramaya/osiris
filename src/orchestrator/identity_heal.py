...
async def heal_contradicting_property(
    actions: Actions, *, object_id: uuid.UUID, name: str, actor: str,
) -> dict[str, Any]:
    ...
    audit never has to guess why a row went quiet. Returns `healed: False` when 0 or 1 current
    ...
    for loser in rows[1:]:
        ...
        result = await retire_assertion(
            ...
            because=(
                ...
                f"{loser['observed_at'].isoformat()}), never sign-off-gated for this "
                "property class — reversible via the retired assertion's own id"))
        if "error" in result:
...
async def reconcile_seat_identity(
    ...
    vacant seat with a stale house)."""
    ...
    healed: dict[str, Any] = {
        "house": await heal_contradicting_property(
            actions, object_id=seat_row["id"], name="house", actor=actor),
    }
    if agent_id is not None:
        ...
        if agent_row is not None:
            healed["project"] = await heal_contradicting_property(
                actions, object_id=agent_row["id"], name="project", actor=actor)
