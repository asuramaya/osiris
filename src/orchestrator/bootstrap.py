...
The generalization of the same migration run on osiris itself: a project's CLAUDE.md build
log, DESIGN.md, and memory/ essays are flat, append-only, re-injected-into-every-context
BLOAT. This ingests them into the shared graph as retrieval-sized `Reference` nodes (dated
history entries, design sections, canon docs), so the knowledge becomes a bounded QUERY
...
migration is the graph's; the file shrink is the agent's, the same split used on osiris.
...
# Repos differ in layout, so this looks in the common PLACES, not one shape.
...
def _plan(path: str, project: str | None) -> tuple[str, list[tuple[str, str, str]], list[str]]:
    ...
    (README/ARCHITECTURE/...) and anything already taken as a log are excluded. Deliberately
    not a blind recursive sweep, which would drag in .venv / node_modules / a public web/
    tree."""
...
`mount(cwd=<this dir>, job_dir=$CLAUDE_JOB_DIR)`, then `orient()`, then write back AS YOU GO
...
async def bootstrap_project(
    ...
    FIXED: every write used to be hardcoded "ref:osiris" regardless of who actually called
    bootstrap, worse than anonymous, a bad injection read as deliberate system canon and
    unable to be traced back to a caller even after the fact.

    `project`, when explicit, goes through the same choke point capture.py's
    `_validate_repo_name` already enforces (reused verbatim rather than re-validated here,
    same law as lineage.py's register_spawn/register_swarm): a path-shaped or otherwise
    malformed caller-supplied name refuses BEFORE any object is touched. The `project or
    root.name` default (`_plan`'s own fallback) is a directory basename, not caller text,
    so it never needs this; the guard only ever fires on an EXPLICIT `project=` argument.

    RESOLVES BY NAME BEFORE MINTING (guards against project identity drift): a bare
    `create_or_find_object` on `repo:{project}` matches only the CANONICAL, frozen at
    whatever label first minted it. Re-running this against a project since renamed (its
    canonical stays `repo:<old>` forever, `rename_project`'s own law) would mint a fresh
    stub under the new label instead of finding the real object, the exact twin-project
    shape live specimens (`repo:handlingtheloop` beside `repo:xxit`) already produced.
    `_resolve_repo` (capture.py, the same choke point `create_project`/`_mint_or_find_repo`
    already trust) is tried first; only a genuine zero-match mints."""
    ...
    for fpath, topic, display in log_specs:
        # THE PROJECT WAS RIGHT HERE (traced from Reference's 37% orphan rate): `proj` was
        # just resolved/created above in THIS SAME CALL, yet every log-chunk Reference this
        # loop mints used to be written with that identity in scope and then discarded it.
        # `repo=project` links each one `in_repo` at the call site instead.
    ...
    migrated = (
        ...
        "ADDITIVE: this is a second, queryable index over your docs, and it never replaces "
        "or removes them. The one file to shrink is CLAUDE.md (write the suggested boot "
        "sector) so its history stops loading into every context; your source docs STAY on "
        "disk, and Osiris never touches your files. Then mount() and orient()."
    )
    return {
        "project": project,
        "registered": str(proj),
...
def main() -> None:  # pragma: no cover - CLI
    ...
    async def run() -> None:
        pool = await create_pool(
            get_settings().database_url, application_name="osiris-script:bootstrap-orchestrator")
        try:
