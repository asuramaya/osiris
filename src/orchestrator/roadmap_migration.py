...
objects. DRY-RUN BY DEFAULT: `plan_migration` performs zero writes; `apply_migration`
exists, is gated, and is never called by this module's own `__main__` block. Applying is
a separate, explicit, future authorization ("nothing applied" until then).

SCOPE: exactly one board. The four-store divergence premise that would have made this a
cross-store reconciler was measured and retracted, there is nothing cross-store to
identify or merge. `plan_migration` takes rows already shaped like the harness tool's own
JSON ({"id","subject","description","status","blocks","blockedBy"}) plus the `store` they
came from, the same "caller supplies the data" discipline task_sync.py already
established; this module never touches ~/.claude/tasks.

WHY COMPLETED ROWS BECOME DECISIONS, NOT THREADS: 133 of 153 rows on the one board
measured are `completed`, median 1338 description chars, up to 4493, a decision log that
has been living in a task list, not a list of open duties. Minting 133 pre-resolved
Thread(kind='obligation') objects would misrepresent settled history as live work on
every future roadmap/orient() read. Only non-completed rows are Thread-shaped.
"""
...
# Hand-classified against capture.ARCS ("honest empty over confident wrong") by reading
# each of the one board's 20 live (non-completed) rows' own subject+description, not
# keyword-inferred. Genuinely dual-fit or off-taxonomy rows are DELIBERATELY ABSENT: an
# id with no entry here mints arc-less, exactly like an unsorted rate measured on a prior
# hand-classified sample. Completed rows (-> Decisions) never consult this map; ARCS is a
# Thread-only taxonomy.
...
def plan_migration(tasks: list[dict[str, Any]], *, store: str) -> dict[str, Any]:
    ...
    where present, a legacy_task_dependencies payload (the blocks/blockedBy DAG is
    preserved as inert data, no verb reads it yet, named not invented). Every plan entry
    is independently applicable; nothing here decides ORDER.
...
def aggregate_line(counts: dict[str, int]) -> str:
    """One line, never per-row noise (the batch-volume rule): "migrated N,
...
def run_aggregate_line(counts: dict[str, int], results: list[dict[str, Any]]) -> str:
    """One line for an actual `apply_migration` run: plan_migration's own aggregate_line
    plus K, the count of times the open_thread dedup-discard signature actually fired and
    had to be repaired via reclassify_thread, the ONLY place this is ever observable, so
    it must be measured every run rather than assumed rare from this one design pass."""
...
async def apply_migration(
    ...
    authorization ("nothing applied" until then). Executes `plan_migration`'s output for
    ...
    READS BACK EVERY THREAD MINT, NEVER TRUSTS THE RECEIPT: open_thread's near-duplicate
    collision path returns the EXISTING thread's id with `deduped: "true"` and silently
    DISCARDS this call's own `arc`, a real prior run saw 17 stamps attempted, 17
    successful-looking receipts, zero landed, caught only by re-counting. After each
    open_thread call this function re-reads that thread's OWN current `arc` property
    directly; if it doesn't match what was requested (the dedup-discard signature), it
    re-applies the arc through `reclassify_thread` (the merged backfill entry point)
