"""deploy_status: the settings pane's deploy section. "deploy snapshot sha vs
deployed.sha" had no read entry point at all before this. Two facts, both read live
off the filesystem, never fabricated:

  running_sha        the git sha THIS repo checkout (wherever the API process's own
                      source lives) currently has checked out, via `git rev-parse HEAD`
  ...
  deploy_snapshot_sha the git sha scripts/update_deploy_snapshot.sh's own pinned
                      worktree is checked out to, using the same `OSIRIS_DEPLOY_SNAPSHOT_DIR`
...
exist (no snapshot ever pinned, a dev worktree, CI): the same "unavailable, not silent"
...
(deploy is the one sanctioned hand that writes machine files)."""
