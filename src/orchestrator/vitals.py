...
Operator ruling, 2026-07-19: the chrome and the harness disagreed on briefs, mail, and
owed counts, because each surface carried its own COPY of each count's SQL, written
...
# base (a whisper echoes the sid back for visitors), or an active object stands behind
# the id. A live visitor (a bg-pty host, a spare) is real and is counted BESIDE the
...
# a resume bridge, is ONE agent (the fold this fixes: fleet once counted 7 agents when
# really 4 were live). The suffix strip folds generations to their base, SOUL_SQL_TEMPLATE
# (agents.py) so this shares the roman-AND-g<N> alternation with every
...
async def live_souls(db: _DB, *, live_secs: int = 900) -> dict[str, int]:
    """{souls, visitors}: distinct live agents (seated) and distinct live visitors.

    CACHE-BASED, CONFESSED, DELIBERATELY NOT ROUTED THROUGH THE HARNESS AUTHORITY (entry-point
    census item 2): `agent_mounts.last_seen`
    ...
    subprocess+/proc census on that busy a path. The obligation this leaves is DISCLOSURE,
...
async def operator_debts(db: _DB, *, hood: str | None = None) -> dict[str, int]:
    """{owed, owed_here}, the RED NUMBER: open threads an agent deliberately placed on the
...
# THE VISIT CLASS'S OWN READ-SIDE PREDICATE (the Great Fold's read-side
# adoption): greatfold.py's `demote_visit_families` marks a registration-only family
# `agent_class='visit'`, a DOORBELL RING, never an agent, never folded into a soul. A
...
async def agent_class_counts(db: _DB) -> dict[str, int]:
    ...
    Every headline that counts agents deflates visit-class agents out of
