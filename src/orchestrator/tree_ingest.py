"""Self-healing tree ingest (operator ruling: tree ingest needs to be a SELF-HEALING infra
thing agents can do by themselves, persistent and tooled, not an osiris-applied bandaid). The
census this needs already existed, tested, with zero callers (neighborhoods.discover_trees):
the missing piece was never the read, it was an ENTRY POINT any seat can act through, and a
heartbeat that notices without acting unattended.

`ingest_project` is that entry point: any seat's own self-service function for its OWN project
(mirrors reconcile_seat_identity's authority shape exactly: self-service derives its target
from the caller's own mount, the third-party sibling takes an explicit target plus a required
reason), combining `gitlog.ingest_repo` (idempotent, always writes when not dry_run) with
...
Cold by default (osiris_tree_ingest_alarm_enabled), same rule as every other scheduled writer
in this house.

THE MAIL-TO-THREAD SHAPE FIX (operator complaint: a condition that is STILL TRUE belongs in a
thread, not mail): this used to `send_message(..., grade='ask')` on every cooldown-cleared
tick, so a tree that stayed blind for 14 days sent 14 byte-identical graded asks, causing
grade inflation (mount()/orient() reported "14 ask" when it meant one), fleet-wide
unread-count degradation, and no self-clearing (the first alarm sat unsettled even after the
tree was ingested on day two). Now: the FIRST time a tree is seen blind,
`open_or_annotate_persisting_alarm` (capture.py's own shared mint-or-annotate primitive,
promoted there from deploy_guard.py once this module became its second real caller, never a
second copy of the logic: the tree-ingest alarm shares its exact shape with schema-drift/
unreviewed-boot, a periodic, source-not-a-human caller re-running on the SAME condition)
mints the Thread AND fires the one-and-only graded 'ask' DM;
every later tick that still finds the SAME tree blind (past its own unchanged 24h cooldown;
the 24h per-tree cooldown is BY DESIGN and should not be shortened) only ANNOTATES the
existing thread, no mail, ever, on a re-assertion. The moment a previously-alarmed tree
stops being actionable (its commits land, by whatever hand), this tick resolves that tree's
own still-open thread itself, the self-clearing behavior this fix added. The earlier lack of
dedupe on condition was always the symptom, not a defect of its own: the thread's own
idempotent identity IS the dedupe now."""
...
def _alarm_summary(tree: str) -> str:
    ...
    path, no watermark), same rule `open_or_annotate_persisting_alarm`'s own docstring
...
async def ingest_project(
    ...
    """THE SELF-SERVICE FUNCTION: land one tree's git history and close the threads it
    witnesses, one call. `project` is a bare repo name (or already-prefixed `repo:...`) and
    must resolve to an ACTIVE SoftwareProject with a usable `on_disk_path` (census_trees's
    own write, or a future explicit registration); refuses loudly otherwise, never guessing
    a path from a name.
    ...
    ingesting them first, and this result says so rather than implying otherwise).

    `dry_run=False` actually lands it: `ingest_repo` then `close_by_commits(dry_run=False)`,
    same repo, one result naming both."""
...
async def ingest_project_third_party(
    ...
    the self-service function, same result shape."""
...
async def uningested_trees_alarm_tick(
    ...
    function always held; the cadence itself was never the bug this fix addressed).
    Never ingests anything itself.

    THE SHAPE: the FIRST time a tree's own Thread is minted, one graded 'ask' DM fires
    alongside it, since a human or agent should be told a new duty exists. Every later
    ...
    behavior added to fix the earlier defect where a resolved condition left its thread open.
    ...
    for r in actionable:
        ...
        if first_notice:
            body = (f"[tree-ingest-alarm] {r['tree']} is on disk at {r['path']} with zero "
                    ...
                    f"ingest_project(dry_run=True) for a preview, then dry_run=False when it "
    ...
    # SELF-CLEARING: a tree that no longer reads zero-commits (whoever ingested it) resolves
    # its own still-open alarm Thread here, rather than leaving the first notice unsettled
    # forever once the underlying condition has cleared.
    for r in rows:
