...
extended to cover the restic repository password, which the offload runner needs).
Reuses `src.ingest.systemd_credential`'s own subprocess boundary (the same module
`soul_crypto.py` was refactored onto for this exact reason) rather than a second
hand-copy of the systemd-creds invocation.
...
first key must come from the normal CLI; this replaced an earlier
...
process starts, when present. This is the exact same daemon fast-path
`soul_crypto.get_soul_key` uses, no subprocess at read time. When absent, the unit
still starts (ImportCredential= tolerates a missing entry, confirmed live), and this
module's own `get_restic_password` raises `ResticPasswordMissing` for the caller to
log loudly rather than crash the whole runner."""
...
def _password_file_path(*, explicit: str | None = None) -> Path:
    ...
    password`). This box only ever runs the systemd --user deploy shape, the same
    fact `soul_crypto.py`'s own resolution ladder was built against, so unlike
    `soul_crypto._key_file_path` there is no root/system-unit branch to carry
    here. XDG-aware (not a hard-coded `~/.config`) so the same `XDG_CONFIG_HOME`
    redirect a test uses to keep the credstore blob (`_credential_path`) off a
    developer's own real box also keeps this logical path's own `.meta.json`
    sidecar off it: a stray sidecar was once found sitting in a real
    `~/.config/osiris/` during this module's own build, and cleaned up by hand."""
    if explicit:
...
def _credential_path(password_path: Path, *, explicit: bool = False) -> Path:
    ...
    default (the first key must come from the normal CLI; same shape as
    `soul_crypto._credential_path`, see that function's own docstring for the
    full live-measured reasoning). EXPLICIT (`explicit=True`, a caller-given
    `--path`/`path=`): the old sibling-of-path shape (`<password_path>.cred`),
    a documented override. Strictly one or the other, never both checked, the
    same single-ladder discipline `_password_file_path` itself already holds."""
    if explicit:
