"""smoke: the deploy-time liveness check (operator ruling). Static gates (ruff/mypy/pytest)
proved they cannot catch an event-loop-lifecycle bug: `_boot_check` warming the wrong pool
(now fixed) shipped past all of them and only broke at real server boot. This module is the
honest, per-surface answer to "is it actually up": one real query over the pool a caller
actually gets (the exact class of bug that fix addressed), and one GET per UI route, never
silently skipped.

Kept deliberately simple: no retries, no thresholds, no alarms of its own. A probe reports
what it saw, and the caller (the MCP tool wrapper, or the CLI) decides what to do about it."""
...
# every UI route this module tracks. /health isn't here on purpose: it's already its own
# dedicated liveness endpoint (src/api/app.py), this walks the actual RENDERED pages.
# The old landing route was retired: "/" (THE INBOX) is :8011's new main entry point.
# /live-desk and /roadmap were retired: pure duplicates of the "live-desk" and "roadmap"
# compositions already roomed in /ui, verified live before deletion.
# /canon was retired 2026-07-30: a pure pass-through to the "docs" composition, whose one
# distinct capability (fixed topic order) became the `sequence` op and was verified live
# in /ui before deletion.
...
async def smoke_chrome(
    ...
    A timed-out route is named explicitly (live measured: `/` at 7.13s past a 5s client
    timeout) rather than falling into the generic error branch below: httpx's own timeout
    exceptions (ReadTimeout/ConnectTimeout) carry an EMPTY str() when raised with no
...
async def smoke_pool(pool: asyncpg.Pool) -> str:
    ...
    other tool fine but fails here would be the exact silent-until-boot bug this check
    exists to catch."""
    try:
...
async def whisper_health(pool: asyncpg.Pool, *, window_hours: int = 24) -> dict[str, Any]:
    """READ-ONLY (kept simple, same rule as every other probe here): the whisper/session-end/
    precompact/stophook hooks each file a failure into the EXISTING blind-spot channel
    (`capture.record_hook_failure`) when they fail; this only reads that back, adding no
    new counter or table. `error_count` is the number of failure assertions across
    `capture.HOOK_ALARM_SURFACES` within `window_hours`; `last_error` names which surface,
    what it said, and when. `ok` is `error_count == 0`: a hook that never failed in the
    window leaves nothing here to find, same as a route that was never hit. This is NOT a
    positive probe (it cannot prove the whisper hook is UP, only that it hasn't confessed
    to being down); `cmd_deploy`'s own synthetic /automount check is the closest thing to
    an active probe."""
...
async def embed_health(pool: asyncpg.Pool, *, window_hours: int = 24) -> dict[str, Any]:
    """READ-ONLY, same rule and same shape as `whisper_health` above: reads
    `capture.EMBED_ALARM_SURFACE` back from the SAME blind-spot channel that `embed_pass`
    (src/workers/arq_worker.py) files into on a load failure, guarding against the semantic
    route silently latching closed forever with nothing watching. `ok` is `error_count == 0`
    in the window; `last_error` names what the most recent failed tick said. NOT a positive
    probe (cannot prove the route is open, only that nothing has confessed to it being
    closed recently)."""
...
async def registry_rowless_warning(pool: asyncpg.Pool) -> str | None:
    """WARNING, never a failure (a whisper-health follow-through): reads
    `mounts.registry_census`'s own `rowless` population back: verified-live harness
    processes (real /proc-confirmed pids) with NO `agent_mounts` row at all, exactly the
    population this check exists to close to zero. Folding "N listed bodies with no row"
    into smoke's own verdict means anyone looking at an unfamiliar deploy sees it in the
    first minute, same rule as `whisper_health` above: a read-back, no new counter.
    Never gates `ok`: a rowless process self-heals at its own next mount (registry_census's
    ...
    alarm stacked on an already-reported gap). Same fail-open rule as every other probe
...
async def smoke(client: httpx.AsyncClient, pool: asyncpg.Pool) -> dict[str, Any]:
    """The whole picture: every UI route plus the pool plus the whisper/hook alarm channel
    plus the registry rowless count plus the semantic-embed alarm channel, composed. `ok`
    is a single boolean a deploy script can branch on without re-deriving the per-surface
...
async def call_mcp_smoke(url: str) -> dict[str, Any] | str:
    """The CLIENT-side half (`osiris smoke`, and scripts/osiris_smoke.py before it):
...
def summarize_failures(chrome: dict[str, str], mcp_result: dict[str, Any] | str) -> list[str]:
    """The two probes (a direct UI walk, the MCP round-trip's OWN view of both the UI and
