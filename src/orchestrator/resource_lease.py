"""Resource leases: an exact-key lock for anything a fleet of agents can
step on at once outside the git index (a file path, the docker daemon, a merge back into
composer).

`open_thread(assignee=)`'s `leased_to` already ships
...
Scope: core + storage only. The MCP verb
that exposes this to the fleet lands separately, once src/mcp_server.py is free.

NARROWED after this module's re-scope ("treat the agents like people working on a
repo"): the working tree was this module's original motivating case (early git races),
...
class LeaseResult:
    ...
    `holder` argument iff `acquired` is True, someone else's otherwise. The lease UX:
...
async def release(pool: asyncpg.Pool, resource_id: str, holder: str) -> bool:
    ...
    THREAD RE-SYNC, ENFORCED: a
    released `resource_leases` row used to leave its companion Thread's status='open' and
    owner=<holder> FOREVER. That was correct for `check_lease` (reads this table directly,
    the live authority) but WRONG for every graph-level reader that only ever sees Thread
    objects, never this SQL table (orient()'s open_threads, recall, briefing, dossier: a
    released lease read as a permanently-open obligation nobody was actually still holding).
    A successful release now also `resolve_thread`s the paired Thread, in the SAME
...
async def reap_stale(pool: asyncpg.Pool, *, older_than_secs: int = 3600) -> int:
    ...
    FLOORED: `older_than_secs` used to be fully caller-controlled with no minimum.
    ...
    (a fleet-wide single point of failure must never refuse-to-serve on a
    ...
    THREAD RE-SYNC (same pattern as `release`'s own fix): a
