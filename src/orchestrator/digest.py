...
surfaces, with no new writes:
  ...
  * ACTIVITY: what the fleet decided or opened in your name since the window opened
    (agent-authored Decisions/Threads only; the miner's derived backfill is excluded, since
    this stream is deliberate work only).
  * DANGER MAP: which agents were model-swapped, i.e. silently demoted by the harness, where
    the harness got nervous.
  ...
  * PROPOSALS: made/accepted/rejected/expired-in-effect per (miner, owner) pair, off the
    ...
    re-estimated here, since it is already the `costs` stream above, off `ceiling()`'s own
    measured vendor figure, and llm_usage carries no owner dimension to split it by pair, so
    this stream doesn't fabricate one.

Read-side by default; the window is either an explicit rolling `since` or the stored operator
watermark ("what's new since I last looked"). Reading never advances the watermark; advancing
is a deliberate `mark_seen` act (a peek must not change state). This is where credence_props
finally meets a live surface.
"""
...
# The operator's digest watermark lives in the generic cursor store (watermarks table,
# migration 0006); its (key, cursor, updated_at) shape fits exactly: `cursor` holds the
# last-seen instant as ISO text, `updated_at` gives the watermark's own age. One row, keyed
...
async def _roster(actions: Actions) -> list[dict[str, Any]]:
    """Every Agent with its model/project and the two health signals: identity_resolved (did
    the onboarding get a clean id?) and model_swapped (was it silently demoted?)."""
    # Each scalar subquery takes the grade-then-recency winner (ORDER BY ... LIMIT 1). A bare
    # subquery assumes <=1 row per (agent, property) and would 500 the whole digest the moment
    # any Agent property lands from two sources.
...
def _last_seen(row: Any) -> datetime | None:
    ...
    aliveness. It is honest enough to window a digest by; it is not honest enough to declare a
    mind dead, and nothing here does.)
    """
...
async def _activity(actions: Actions, since: datetime, limit: int = 50) -> list[dict[str, Any]]:
    ...
    self_declared Decisions/Threads. Two exclusions keep this the deliberate-work stream:
      * the miner's derived backfill is now sourced to the originating agent too (origin
        attribution), so `agent:%` alone no longer separates it; the grade does instead: only
...
async def _credence_streams(
    ...
    """One credence_props pass over objects more than one agent co-asserted in the window,
    split into its two upward streams: laundering (a relay carrying a fact above its origin
    grade) and disputes (an ancestor whose value genuinely differs from its subtree's origin).
    Both empty while co-assertion is nascent; this is the wire that makes credence bite as the
    fleet grows."""
    # Candidates = objects an agent touched in the window whose (object,name) also carries
    # another agent source all-time. The archetypal laundering re-reports an older origin
    # (origin before the window, relay inside it); requiring both sources inside the window
...
async def _operator_inbox(actions: Actions, *, lease_secs: int) -> dict[str, Any]:
    ...
    The supersession tension, resolved as presentation: an agent that updates its prior brief
    (send(reply_to=<its own brief>), the threading duty) stacks the thread; the desk shows only
    the newest unread brief per thread, with the older ones counted under it (`supersedes`),
    and `unread` counts active heads, not raw rows. The system folds; only the human settles:
    nothing here leases or acks (a peek), and the superseded briefs stay unread underneath
    until the operator's explicit word clears the thread."""
...
async def _resolve_since(
    ...
    An explicit `since` is an ad-hoc rolling window (mode='explicit'); the watermark is
    untouched and unread. `since=None` is watermark mode: the lower bound is the stored
    operator watermark ('what's new since I last looked'), or a 24h fallback when none has
    ever been set."""
...
async def _costs(actions: Actions, since: datetime) -> dict[str, Any]:
    ...
    number never overclaims (answers 'where are the tokens burnt', metered)."""
    # Dollars only when they are real: on a subscription the CLI's cost_usd is notional, so
    # the console shows the real token counts and omits the phantom dollar figure, the same
    # reason the daily ceiling no longer gates on it. Tokens are always real.
...
async def _bodies(actions: Actions, since: datetime) -> dict[str, Any]:
    ...
    # set (exit:N for any N), so summing the limited groups would assert a prefix as the
    # whole, the same undercounting trap the COST stream's coverage note calls out.
...
async def _proposal_telemetry(actions: Actions, since: datetime) -> dict[str, Any]:
    """Made/accepted/rejected/expired-in-effect per (miner, owner) pair, off the Proposal
    objects propose()/accept()/reject() already mint (proposals.py). `made` counts a Proposal
    whose own `miner` property (asserted once, at mint) falls in the window;
    `accepted`/`rejected` count a `status` transition (supersede_assertion, so its own
    `observed_at` is the transition time, not the mint time) in the window; `expired` is a
    live snapshot, still `status='proposed'` but past its own `expires_at`, since nothing
    sweeps a Proposal to a real 'expired' status yet. No dollar figure here: see the module
    docstring's PROPOSALS note.

    `by_lane`: the weekly desk line reports proposals and acceptance per lane. A lane is
    `<abstained object's own type>:<link_type>`, read off each Proposal's own
    `evidence_pointer` (from_id/link_type), never a new column, since the abstention miner's
    own lane table is keyed by exactly this pair already. Generic on purpose (not
    abstention-miner-specific): any future propose() caller whose evidence_pointer resolves
    cleanly gets grouped the same way, for free.

    `by_lane[*]["by_signal"]`: a made-in-window Proposal's own `candidate.signal` (the
    abstention miner's own "dominance"/"author_tiebreak" naming, opt-in; a `candidate` with
    no `signal` key, any Proposal from before this was added or any non-abstention caller,
    contributes nothing here) counted per lane, so the desk can see which signal is actually
...
async def _retrieval(actions: Actions, since: datetime) -> dict[str, Any]:
    ...
    # actually carry recall, the quality telemetry the search engine is judged by
    # (zero-hits alone was retired as the sole tripwire since it undercounted this).
...
async def _miner(actions: Actions, since: datetime) -> dict[str, Any]:
    """Sensing-tick health off miner:ticks telemetry: the instrument a prior outage
    demanded, when a fail-open cron died for a day behind a green heartbeat. Shows whether
    ticks finish, how long they run, how saturated the LLM budget is, and how far behind the
    fleet's transcripts the miner sits."""
    ...
    out: dict[str, Any] = {
        ...
        # Starts that never recorded a completion: timeout cancels that outran the
...
# Obligation-backlog hygiene: osiris itself is not a client project and carries a wider
# target; every other project is a client.
...
async def _open_obligation_rows(actions: Actions) -> list[dict[str, Any]]:
    ...
    (per-project) and `obligation_backlog` (per-project and per-seat), extracted so the two
    never hand-roll two copies of the same WHERE clause to drift apart.

    `contested`: present and `True` when a note newer than this row's own `summary` disputes
    it; the backlog band's own consumer marks this rather than quoting a disputed headline as
    settled."""
...
async def _obligation_pressure(actions: Actions) -> list[dict[str, Any]]:
    """The obligation-backlog hygiene weekly gauge: every project carrying at least one open
    kind='obligation' Thread, against its fixed target (osiris itself under
    `_OBLIGATION_TARGET_OSIRIS`, every client project under `_OBLIGATION_TARGET_CLIENT`),
    naming the three oldest owners (the ones who have been carrying it longest) rather than a
    bare count alone; a digest that only says "47 open" tells nobody whom to ask, naming the
    oldest three points at exactly that. `(unfiled)` (no in_repo link at all) is its own
    bucket, same convention `obligation_hygiene.hygiene_status` already uses, and is never
    scored against a target (an unfiled obligation is a filing gap, not a project's own
    pressure).

    `past_window` (the `backlog` verb's own addition): how many of a project's open
    obligations are already past their own `stale_after` window (same definition
    `owned_obligations`/`compute_stale_obligations` in stophook_logic.py use), 0 for a row
    with none, or every row whose obligations predate the stale_after_days migration (no
    window stamped at all)."""
...
async def fleet_digest(
    ...
    # the graph, but real and current). Both mean the harness got nervous; the operator must
    ...
    # Danger = what the operator can still act on: a swap on a mind seen inside the window, or
    # a live swap the heartbeat caught unstamped (current by definition). A swap on an agent
    # the graph has never seen is an artifact of a fleet that has run for months (91 of them).
    # Showing all 91 at every glance would rebuild an unreadable, permanently-red desk: a
    # permanent, unactionable number teaches you to stop reading it. They are counted below
    ...
    return {
        ...
        "summary": {
            ...
            # wake jobs, and 162 whose transcripts the disk no longer has. A ghost is a
            # succession gap instead, work continued with no handoff, which is a question
            # about links, not about liveness. Do not read one for the other.
