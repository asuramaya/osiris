...
categories and canonical schemes, the vocabulary a human author uses to build a composition or
read a result against. It says nothing about a table's actual Postgres columns, types, or
indexes, because it was never meant to; that is a different question, at a different layer, and
the graph never had a verb for it. The corpus's own numbers made the gap concrete: 80
hand-rolled introspections and 45 failed queries in one report window, four of them within 83
seconds on a single day, a human reaching for the schema tool, finding it silent on physical
shape, and going straight to `information_schema` by hand instead.
