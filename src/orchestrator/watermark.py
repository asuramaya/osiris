"""THE GRAPH WATERMARK: auto-refresh's whole mechanism is to poll a cheap change-marker,
never the composition itself. Re-running the composition every N seconds fails in both
directions: idle, it burns real work for nothing (the roadmap composition measured 61K
chars unbounded); under real write load it re-runs the full spec on every tick per open
tab. It also collides with a known finding about the census code: census.live_bodies() is
a synchronous pgrep plus a /proc read, so anything touching it on a fast loop blocks the
event loop, fleet-wide, per tab. A watermark that only tells the client "something moved"
costs nothing when nothing did and never runs an expensive function on a timer it doesn't
need.
...
set that makes the two "seconds" compositions this design targets, mail and fleet-strip,
and the general knowledge graph actually live):
  ...
  - agent_mounts.mounted_at (timestamptz, set once at INSERT and never touched again;
    ...
    last_seen updates on every heartbeat ping from every live session, so using last_seen
    here would make the marker move constantly regardless of whether anything a human
    would call "a change" actually happened.
  - agent_wakes.id (bigserial, PK-indexed): the wake ledger that fleet-strip's own pulse
    line (mounts.fleet_pulse, wired through _fn_fleet_pulse_line) reflects.

A later fix addressed a second gap: auto-refresh covered the LENS (the composition CONTENT
on screen) but nothing told the client the CATALOG had moved. The room switcher's own
`cases`/`compositions` counts (index.html's loadRooms) and the composer sidebar's shelf of
lenses (loadComps) are fetched once at boot/room-switch and never revisited. The reported
case: a compositions backfill landed (verified, deployed), and the sidebar kept reading a
stale count against a room that had actually grown. THREE MORE MARKERS, same discipline as
the four above, never folded into them, since `rooms`/`compositions`/`cases` are
work-artifact tables outside the Actions/audit_log gate entirely (same reason
fleet_messages needed its own field: a direct INSERT, not a domain write):
  ...
  - compositions.created_at: a new saved composition (lens or watch); this is exactly the
    backfill shape described above. KNOWN GAP, accepted to keep this v1 small (same spirit
    ...
    assumed away, not chased without a live report to justify it.
...
later additions scan rooms/compositions/cases, tables smaller than agent_mounts' own 25
rows in every environment seen so far, so they ride the same non-factor budget rather than
being re-measured from scratch."""
...
async def graph_watermark(pool: asyncpg.Pool) -> dict[str, Any]:
    """{table: its own latest marker}, one per table above. None means a genuinely empty
    ...
    The three later additions (rooms/compositions/cases) are the CATALOG half: the client
    polls them separately from the four above (the LENS half) and, on a move, re-fetches
    /rooms + /compositions rather than re-running whatever composition happens to be on
    screen. Same field, same endpoint, two independent client-side watchers reading
    different keys out of it, not two endpoints, since the query itself is one cheap
