"""THE DOCS COMPILER: a doc's compiled section regenerates from what it actually snapshots,
instead of being a hand-maintained artifact someone must keep true by hand and a test must
assert byte-for-byte (the mechanism this replaces: tests/test_schema.py's retired
REFERENCE.md == render_reference() assertion).
...
exactly the "two records of one truth" shape this house already treats as a defect. The
NEVER-CLOBBER BOUNDARY and REFUSE-ON-MANGLED-MARKER rule are load-bearing here for the
identical reason they are in boot_compiler.py: hand prose living in the SAME file as a
compiled section (REFERENCE.md's evidence-classes/sources/MCP-tools/repo-map prose,
surrounding the compiled data-model block) must survive a recompile byte-for-byte.

REFERENCE.md IS DELIBERATELY POOL-FREE (correcting an earlier mis-stated acceptance
criterion that would have wired it to catalog.py's LIVE, accretive catalog instead): it
compiles from schema.py's STATIC declared manifest, via the EXISTING schema.render_reference()
(not reimplemented here, so there is exactly one function that knows how to render the type
catalog as markdown), so it is regenerable from an empty checkout with no database at all,
and an agent minting a type through ensure_type's live accretion path changes NOTHING in the
shipped doc. `reference_catalog` is ALSO registered as a composition function in
compositions.py (queryable via run_composition("reference"), UI/API parity with every other
named lens), but that async path is for LIVE inspection only; the FILE-COMPILE path below
never touches a pool, on purpose.
...
def compile_markdown_section(
    ...
    # (accumulating forever) instead of being idempotent, so absorb exactly the one newline
