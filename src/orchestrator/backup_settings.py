...
piece 3), folding the write half already built (piece 3b) into
`src/config/settings_registry.py`'s own SettingSpecs, so this domain gets the SAME
validation/authority/effect machinery every other knob does instead of a parallel
...
`scripts/render_units.py` (generalized this from `render_backup_timers.py`, its
...
THE BACKUP TOPOLOGY / INTERMITTENT TARGETS (operator ruling):
...
async def _target_presence(target: dict[str, Any]) -> dict[str, Any] | None:
    """Per-target `present now` (`backup-settings get`/`backup-status`
    both show this). 'local' -> the real live-mount check (`check_local_target_presence`,
    findmnt); 'restic' -> None, deliberately: reachability is a network fact this entry
    point never checks, so there is no honest yes/no to give here at all, only a shape
    verdict at write time."""
    ...
    **fields: Any,
    ...
    if "offload_targets" in fields:
        ...
        if run_shape_check:
            ...
            # restic-grammar check required at write time, same as the
