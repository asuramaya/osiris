"""THE BROWSER RECOVERY MATERIAL ROUTE (THE KEY PANEL, piece 2): the one new
server-side surface piece 2 needs, a single-use, 60-second-expiry handout of the
current primary soul key's raw bytes, so a browser page can wrap them client-side via
WebAuthn PRF (RP id `osiris.local`, HKDF-SHA256 with `info=b"osiris-soul-key-
...
NEW CODE ONLY: no edits to src/ingest/soul_crypto.py or src/orchestrator/soul_key.py.
This module composes only their PUBLIC surface (`soul_key_status`, `read_key_bytes_at`);
the `<path>.recovery.json` naming is reproduced here as one line rather than importing
...
NOT PHYSICALLY VERIFIED BY WHOEVER WROTE THIS (no hands, no eyes on a Security Key, no
browser WebAuthn ceremony ever actually run): flagged explicitly in the tip, matching
the same honesty soul_crypto.py's own FIDO2 section states about its CLI counterpart.
One real enroll+recover cycle against the operator's own hardware, from an actual
...
def recover_from_browser(*, raw_key_b64: str, key_fingerprint: str,
    ...
    would risk a genuine security-critical divergence from the already-tested
    ...
    # single resolution-order discipline soul_crypto._credential_path now holds
    # house-wide (never the old hardcoded sibling-`.cred` shape, which silently
    # misses a credstore-resident key and would let a recovery attempt clobber one).
